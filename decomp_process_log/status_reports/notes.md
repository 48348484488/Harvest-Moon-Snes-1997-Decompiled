...
Added text repoint planning and Bank BB overflow allocation preview tools. The default source remains byte-perfect. See `DECOMP_PASS_05_TEXT_REPOINT.md` and `STATUS_DECOMP_PASS_05.md`.


## Pass 16 — Tool Actions Core

Núcleo de ações de ferramentas documentado: dispatcher, tabelas de animação/efeito, check single/line/square, sementes, regador/sprinkler, ração e ferramentas douradas. Build validada byte-perfect.

## Pass 23 - Input/Menu State Core 100%

Fechado o nucleo de input/menu: dispatcher por !inputstate, name-entry, menu submode $95, prompt cursor, save/load slot selection e escolhas basicas. Build revalidada byte-perfect.

## Pass 24 - Inventory / Tool Menu Core 100%

Escopo fechado nesta pass: nucleo de inventario/ferramenta e display condicional de tool shed/lojas.

Renomeacoes principais:

- `Event_UNKTools` -> `InventoryTool_ReturnHeldToolsToShedBitfields`
- `DATA16_81BE0F` -> `InventoryTool_BitfieldSlotAndPickupTileTable`
- `DATA8_81B363` -> `ShopDisplay_ItemTilePlacementTable`
- `SUB_81B2BB` -> `ShopDisplay_DrawConditionalItemTile`

Build validada byte-perfect com a ROM USA limpa.


## PASS27_PLAYER_MOVEMENT_COLLISION_CORE

Escopo fechado: Player Movement / Collision / Interaction Core 100%.

Foram documentados e renomeados o dispatcher normal de movimento, botoes de acao, calculo de tile em frente e helpers centrais de colisao com tile/objetos. Build byte-perfect mantida.
