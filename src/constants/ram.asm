...
!tilemap_pointer = $18
!tilemap_to_load = $22 ;8b more data on MapLoad_AreaGraphicsPointerTable.asm
...
!Joy2_Unused = $013A
;
!inputstate = $019A ;8b PASS23: central input mode selector. 01=field movement, 02=no-control, 03=prompt cursor, 04=menu, 05=name-entry, 06=extra modal.
...
!temp_name_3 = $0887 ;8b
!temp_name_4 = $0888 ;8b
!name_entry_index = $0994 ;8b PASS23: current character count/index in name-entry mode.
!menu_pos = $0991 ;16b PASS23: generic menu/cursor position used by menu/name-entry flows.
...
!item_on_hand = $091D ;8b PASS24: held item ID currently carried/used; zero means empty hands.
!old_item_on_hand = $091E ;8b
;at the end of TODO, need to find a place for Items
!tool_selected = $0921 ;8b PASS24: active tool slot selected from inventory/tool shed display.
...
!tool_backpack = $0923 ;8b PASS24: secondary/backpack tool slot used by inventory/tool-return flows.
...
!exhaustion_level = $096C ;8b PASS15: stamina fatigue stage, 0..3
...
; PASS20: Family/social persistent bitfields. These aliases name previously raw SRAM mirrors.
; !player_house_and_event_flags contains house/social event bits used by upgrades and marriage/family gates.
!player_house_and_event_flags = $7F1F64 ;16b PASS20: house level and event flags, includes house level 2 bit $0080.
; !marriage_flags contains spouse/marriage identity bits. Observed bits: Maria=$0001, Ann=$0002, Nina=$0004, Ellen=$0008, Eve=$0010.
!marriage_flags = $7F1F66 ;16b PASS20: selected wife / marriage identity bitfield.
; !family_event_flags contains transient family/event notification bits, including pregnancy/about-to-give-birth and child age milestones.
!family_event_flags = $7F1F6C ;16b PASS20: family event/milestone flags.
; !child_flags contains child existence/birth state flags. Observed bits: child1=$0004, child2=$0008, child born/event=$0010.
!child_flags = $7F1F6E ;16b PASS20: child existence and birth-stage flags.
...
!shipping_moneyL = $7F1F07 ;24-bit daily shipping accumulator low word. Added to wallet at night reset.
!shipping_moneyH = $7F1F09 ;24-bit daily shipping accumulator high byte.
!cow_N = $7F1F0A ;8b
!chicks_N = $7F1F0B ;8b
...
!ranch_mastery = $7F1F54 ;16b PASS36: final ranch mastery / ending score accumulator, capped at 999.
!ranch_development = $7F1F56 ;16b PASS36: farm development score, contributes half value to ranch mastery.
!moneyL = $7F1F04 ;wallet money low word. Used with !moneyH as 24-bit value.
!moneyH = $7F1F06 ;wallet money high byte. AddMoney clamps wallet to $0F423F.
...
;; 13 chicken slots, 8 bytes each. Adult hens use daily egg bitmasks in Chicken_EggLaidBitmaskTable.
;; Commonly observed slot layout:
;; +0 status bitfield: exists/egg/chick/adult/incubating/timer state
;; +1 map/location or egg-age value depending on state
;; +2 growth/incubation timer / temporary state timer
