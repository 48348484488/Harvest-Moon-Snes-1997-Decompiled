...
; PASS13_CLOCK_CORE: main overworld clock dispatcher.
; - Checks !time_running bit 1 ($02) to force the nightly day rollover.
; - Checks !time_running bit 0 ($01) to allow/stop the visible clock.
; - Advances seconds every frame/tick, minutes every 60 seconds, hour every 15 minutes.
; - Dispatches noon lunch, 5PM shipping scene, 6PM evening audio/palette transition.
;;;;;;;; Updates current time and calls hour related functions
Clock_UpdateAndDispatchHourlyEvents: ;828000
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.W !time_running
        AND.B #$02                            ;advance to next day
        BEQ +
        JMP.W DayCycle_NightResetAdvanceDate
    ...
    + %Set16bit(!M)
        ...
        AND.W #$0400                          ;FLAG5A
        BEQ +
        JMP.W ShippingScene_ShowDailyEarningsDialogue
    ...
    + %Set8bit(!M)
        ...
        INC A
        STA.L !hour
        JSL.L DayCycle_HaveLunchAtNoon
        JSL.L ShippingScene_StartAt5PM
        %Set8bit(!M)
        LDA.L !hour
        CMP.B #18
        BEQ .hour6PM
    ...
    .noTimeUpdate:
        ...
        PHA
        JSL.L Palette_SetPendingTransitionIfHourChanged
        %Set8bit(!M)
        PLA
        CMP.W !palette_to_load
        BEQ .return
        LDY.W #$0004
        JSL.L PaletteAnim_ClearPointer42SlotsFromIndex         ;TODO
        JSL.L PaletteAnim_RunMapTimeInstaller           ;TODO

    .return:
        RTL

; PASS13_NOON_EVENT / PASS15_STAMINA_RECOVERY: noon lunch trigger.
; Runs only when !hour == 12. Adds 20 stamina and, unless blocked by
; fishing/hopping/action flags, schedules the eating animation/state.
;;;;;;;; Increases stamina and plays eating animation
DayCycle_HaveLunchAtNoon: ;8280AA
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.L !hour
        CMP.B #12
    ...
    .midhop:
        CMP.W #$0018                           ;Mid Hop
        BNE .Bank82_MainLogicBranch_8280FF
        JMP.W .return

    .Bank82_MainLogicBranch_8280FF:
        LDA.B !game_state
        AND.W #$0800                           ;FLAGD2
        BEQ .Bank82_MainLogicBranch_828111
        JMP.W .return

    .Bank82_MainLogicBranch_828111:
        %Set8bit(!M)
        LDA.B #$03
...
; PASS14_SHIPPING_CORE: 5PM shipping collection scene starter.
; Runs from Clock_UpdateAndDispatchHourlyEvents when !hour reaches 17.
; It only starts on farm tilemaps (< $04). It freezes input, sets the
; shipping-scene pending flag at $7F1F5A bit $0400, then loads the CC data
; for the collection cutscene. The actual earnings dialog is shown by the
; next routine after the scene flag transitions to bit $0800.
;;;;;;;; Plays the scene of the sell if you are in the farm. This subrutine
;;;;;;;; flows into the next one
ShippingScene_StartAt5PM: ;828131
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.L !hour
        CMP.B #17
        BEQ .startShippingSceneAt5PM
        JMP.W ShippingReturn

    .startShippingSceneAt5PM:
...
; PASS14_SHIPPING_DIALOGUE: daily shipping dialogue selector.
; If !shipping_moneyL/H is non-zero, uses text $031A (daily shipping total).
; If no goods were shipped, uses text $031B (nothing shipped). This routine
; only opens after $7F1F5A bit $0800 is set and game_state bit $0040 is clear.
;;;;;;;;
ShippingScene_ShowDailyEarningsDialogue: ;828165
        %Set16bit(!M)
        LDA.L $7F1F5A
        AND.W #$0800                        ;FLAG5A
        BEQ ShippingReturn
        ...
        LDA.B !game_state
        AND.W #$0040                        ;FLAGD2
        BEQ .Bank82_MainLogicBranch_82817C
        JMP.W ShippingReturn

    .Bank82_MainLogicBranch_82817C:
        LDA.W #$0002
        STA.W !inputstate                    ;FLAGD2
        LDX.W #$031A                         ;Text_Daily_Shipping
        %Set16bit(!M)
        LDA.L !shipping_moneyL
        BNE .Bank82_MainLogicBranch_828198
        %Set8bit(!M)
        LDA.L !shipping_moneyH
        BNE .Bank82_MainLogicBranch_828198
        LDX.W #$031B                         ;Text_Daily_Shipping_Nothing

    .Bank82_MainLogicBranch_828198:
        %Set8bit(!M)
        LDA.B #$00
        STA.W $0191
        JSL.L TextBox_StartByTextId                      ;TODO
        %Set16bit(!M)
        LDA.W #$0006
        LDX.W #$0000
        LDY.W #$0027
...
Weather_LoadDisplayClimateFromTomorrow: ;8281C0
        ...
        LDA.W $0196
        AND.W #$0200                           ;FLAG196
        BNE .Bank82_MainLogicBranch_8281F3
        BRA .return
    ...
    .Bank82_MainLogicBranch_8281F3: %Set8bit(!M)
        LDA.B #$04
        STA.W $0990
        BRA .return
...
;;;;;;;;
Weather_ApplyFarmDamageForCurrentFlags: ;828209
        %Set16bit(!MX)
        LDA.W $0196
        ...
        LDA.W $0196
        AND.W #$0100                           ;FLAG196
        BNE .Bank82_MainLogicBranch_82825E
        LDA.W $0196
        AND.W #$0200                           ;FLAG196
        BNE .Bank82_MainLogicBranch_828260
    ...
    .raining:
        %Set16bit(!M)
        LDA.W #$0000
        JSR.W Weather_LoadFarmDamageProbabilities
        JSL.L FarmTiles_ApplyClimateDamage
        BRA .return
    ...
    .Bank82_MainLogicBranch_82825E:
        BRA .return

    .Bank82_MainLogicBranch_828260:
        %Set16bit(!M)
        LDA.W #$0003
        JSR.W Weather_LoadFarmDamageProbabilities
        JSL.L FarmTiles_ApplyClimateDamage
...
Weather_LoadFarmDamageProbabilities: ;828270
        %Set16bit(!MX)
        ASL A
        ...
        LDA.B #$00
        XBA
        LDA.L Weather_FarmDamageProbabilityTable,X       ;Fence
        %Set16bit(!M)
        PHA
        INX
        %Set8bit(!M)
        LDA.L Weather_FarmDamageProbabilityTable,X       ;Grass
        %Set16bit(!M)
        PHA
        INX
        %Set8bit(!M)
        LDA.L Weather_FarmDamageProbabilityTable,X       ;Crops
        %Set16bit(!M)
        TAY
        PLX
        PLA
...
Weather_FarmDamageProbabilityTable: ;828298
        db $60,$00,$00,$00  ;Rain
        db $40,$00,$00,$00  ;Snow
...
;;;;;;;;
; PASS13_DAY_ROLLOVER: nightly date/calendar/state reset.
; Advances weekday/day/season/year, applies weather flags and farm damage,
; resets clock to 06:00:00, updates date display buffers, updates farm tiles,
; livestock, tomorrow weather, wife/child progress, stamina and shipping money.
DayCycle_NightResetAdvanceDate: ;8282AC
        %Set8bit(!M)
        %Set16bit(!X)
        %Set8bit(!M)
        LDA.L !weekday
    ...
    .skipflagset:
        JSL.L Weather_ApplyCurrentDayFlags                  ;TODO, something related to normal days
        %Set16bit(!MX)
        LDA.W $0196
        AND.W #$0200                           ;FLAG196
        BEQ .Bank82_MainLogicBranch_828367
        LDA.L !player_house_and_event_flags
        ORA.W #$0010                           ;FLAG64
        STA.L !player_house_and_event_flags

    .Bank82_MainLogicBranch_828367:
        %Set16bit(!MX)
        LDA.W $0196
        AND.W #$0100                           ;FLAG196
        BEQ .Bank82_MainLogicBranch_82837C
        LDA.L $7F1F70
        ORA.W #$0002                           ;FLAG70
        STA.L $7F1F70

    .Bank82_MainLogicBranch_82837C:
        JSL.L $8095F5                          ;TODO
        %Set8bit(!M)
        LDA.B #$06
        STA.L !hour
        ...
        LDA.B #$00
        STA.L !seconds
        JSL.L Calendar_LoadDateNameBuffers
        JSL.L FarmTiles_NightlyGrowthDryingAndCleanup
        JSL.L Weather_ApplyFarmDamageForCurrentFlags
        JSL.L Livestock_DailyStatusAndFeedingUpdate
        JSL.L Weather_RollTomorrow
        JSL.L Romance_UpdateMostLovedGirlNameBuffer
        JSL.L Family_DailyPregnancyAndChildGrowth
        %Set16bit(!M)
        STZ.W $0915
        STZ.B !game_state
        STZ.B !player_action
        ...
        STA.W $0901
        %Set16bit(!M)
        ; PASS14_SHIPPING_DEPOSIT: transfer the accumulated bin value into
        ; wallet money during the nightly reset. AddMoney consumes $72/$74 as
        ; a signed 24-bit delta, clamps the wallet at 999999G and rejects
        ; negative results. After this transfer, the daily shipping accumulator
        ...
        AND.W #$0020
        BEQ .Bank82_MainLogicBranch_828482                       ;TODO
        %Set16bit(!M)
        LDA.L !marriage_flags
        AND.W #$FFDF                           ;FLAG66
    ...
    .skiphoeevent:
        ...
        LDA.L $7F1F6A
        AND.W #$0400                           ;Flag6A
        BEQ .Bank82_MainLogicBranch_8285A9
        LDA.L $7F1F6A
        AND.W #$0800                           ;Flag6A
        BNE .Bank82_MainLogicBranch_8285A9
        LDA.L $7F1F6A
        ORA.W #$0800                           ;Flag6A
    ...
    .Bank82_MainLogicBranch_8285A9:
        %Set16bit(!MX)
        LDA.L $7F1F6A
        AND.W #$1000                           ;Flag6A
        BEQ .Bank82_MainLogicBranch_8285E2
        LDA.L $7F1F6A
        AND.W #$2000                           ;Flag6A
        BNE .Bank82_MainLogicBranch_8285E2
        %Set8bit(!M)
        LDA.W $09A3
        INC A
        STA.W $09A3
    ...
    .Bank82_MainLogicBranch_8285E2:
        %Set16bit(!MX)
        LDA.L $7F1F6A
        AND.W #$4000                           ;Flag6A
        BEQ .Bank82_MainLogicBranch_82861B
        LDA.L $7F1F6A
        AND.W #$8000                           ;Flag6A
        BNE .Bank82_MainLogicBranch_82861B
        %Set8bit(!M)
        LDA.W $09A3
        INC A
        STA.W $09A3
    ...
    .Bank82_MainLogicBranch_82861B:
        %Set16bit(!MX)
        LDA.L !family_event_flags
        AND.W #$0001                           ;Flag6C
        BEQ .Bank82_MainLogicBranch_828654
        LDA.L !family_event_flags
        AND.W #$0002                           ;Flag6C
        BNE .Bank82_MainLogicBranch_828654
        %Set8bit(!M)
        LDA.W $09A3
        INC A
        STA.W $09A3
    ...
    .Bank82_MainLogicBranch_828654:
        %Set16bit(!MX)
        LDA.L !family_event_flags
        AND.W #$0004                           ;Flag6C
        BEQ .Bank82_MainLogicBranch_82868D
        LDA.L !family_event_flags
        AND.W #$0008                           ;Flag6C
        BNE .Bank82_MainLogicBranch_82868D
        %Set8bit(!M)
        LDA.W $09A3
        INC A
        STA.W $09A3
    ...
    .goldhoereturn:
        ...
        LDA.L !child_flags
        AND.W #$8000                           ;FLAG6E
        BEQ .Bank82_MainLogicBranch_8286E8
        LDA.L !child_flags
        AND.W #$7FFF                           ;FLAG6E
        STA.L !child_flags
        %Set8bit(!M)
...
;;;;;;;; Infinite Loop???
Debug_InfiniteLoopBugTrap: BRA Debug_InfiniteLoopBugTrap                 ;BUG

;;;;;;;; PASS20_FAMILY_CORE: daily wife pregnancy and child growth pipeline.
Family_DailyPregnancyAndChildGrowth: ;828790
        %Set16bit(!MX)
        LDA.L !family_event_flags
        AND.W #$FC7F
        STA.L !family_event_flags
    ...
    .abouttogivebirth:
        ...
        + LDA.L !marriage_flags                          ;Eve was totally an aftertough
        AND.W #$0010
        BEQ +
        JMP.W .wifeeve
...
;;;;;;;;
; PASS13_DATE_TEXT: loads season, weekday and ordinal suffix display buffers.
; Season index uses 16-byte rows; weekday index uses 32-byte rows; ordinal
; suffix uses special rows for 1st/2nd/3rd and default row for all other days.
Calendar_LoadDateNameBuffers: ;8289D6
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.B #$00
        XBA
...
Season_Names_Table: ;828AD9
        dw $002C,$000F,$0011,$0008,$000D,$0006,$0000,$0000
        dw $002C,$0014,$000C,$000C,$0004,$0011,$0000,$0000
        dw $001F,$0000,$000B,$000B,$00B1,$00B1,$0000,$0000
        dw $0030,$0008,$000D,$0013,$0004,$0011,$0000,$0000

;;;;;;;; This is text, it holds the name of the season and spaces at the end
Weekday_Names_Table: ;828B19
        dw $002C,$0014,$000D,$0003,$0000,$0018,$00B1,$00B1
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $0026,$000E,$000D,$0003,$0000,$0018,$00B1,$00B1
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $002D,$0014,$0004,$0012,$0003,$0000,$0018,$00B1
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $0030,$0004,$0003,$000D,$0004,$0012,$0003,$0000
        dw $0018,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $002D,$0007,$0014,$0011,$0012,$0003,$0000,$0018
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $001F,$0011,$0008,$0003,$0000,$0018,$00B1,$00B1
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000
        dw $002C,$0000,$0013,$0014,$0011,$0003,$0000,$0018
        dw $00B1,$0000,$0000,$0000,$0000,$0000,$0000,$0000

;;;;;;;; This is text, it holds the ordinals of the day
Ordinals_Names_Table: ;828BF9
        dw $0012,$0013,$000D,$0003,$0011,$0003,$0013,$0007
;;;;;;;;
; PASS13_WEATHER_FLAGS: converts !weather_tomorrow into current-day flags.
; This updates $0196 climate flags and some event/festival flags before the
; day begins. Weather_RollTomorrow then chooses the next day forecast.
Weather_ApplyCurrentDayFlags: ;828C09
        %Set16bit(!MX)
        STZ.W $0196                            ;FLAG196
        %Set8bit(!M)
        LDA.B #$00
    ...
    .notsummer:
        %Set16bit(!M)
        LDA.L Weather_CurrentFlagTable               ;FLAG196
        STA.W $0196
        JMP.W .return
    ...
    .rain:
        ...
        %Set16bit(!M)
        LDX.W #$0004
        LDA.L Weather_CurrentFlagTable,X             ;FLAG196
        STA.W $0196
        JMP.W .return
...
;;;;;;;;
Weather_CurrentFlagTable: ;828CED
        dw $0004,$0002,$0008,$0010,$0200,$0100

;;;;;;;; Forecast/festival scheduler for tomorrow.
; PASS19_FESTIVAL_WEATHER_CORE_100:
; 1) checks fixed festival dates first and writes a special weather/festival ID;
; 2) applies special first-year winter/fair/thunder cases;
; 3) falls back to season RNG tables for hurricane, rain and snow.
; This routine is the central calendar-to-weather/festival bridge.
Weather_RollTomorrow: ;828CF9
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.L !season
        BNE .notspring
        LDA.L !day
        CMP.B !DAY_FLOWER_FESTIVAL
        BEQ .flowerfestival
        JMP.W .normalday

    .flowerfestival:
        %Set8bit(!M)
        LDA.B !WEATHER_FLOWER_FESTIVAL        ;spring day 22 fixed festival climate
    ...
    .notspring:
        ...
        BNE .notfall
        LDA.L !day
        CMP.B !DAY_HARVEST_FESTIVAL
        BNE .notharvestfestival
        %Set8bit(!M)
        LDA.B !WEATHER_HARVEST_FESTIVAL       ;fall day 11 fixed festival climate
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .notharvestfestival:
        %Set8bit(!M)
        LDA.L !day
        CMP.B !DAY_EGG_FESTIVAL
        BEQ .eggfestival
        JMP.W .normalday

    .eggfestival:
        %Set8bit(!M)
        LDA.B !WEATHER_EGG_FESTIVAL           ;fall day 19 fixed festival climate
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .iswinter:
        LDA.L !day
        CMP.B !DAY_THANKSGIVING
        BNE .notthanksgiving
        %Set8bit(!M)
        LDA.B !WEATHER_THANKSGIVING          ;winter day 9 fixed festival climate
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .notthanksgiving:
        %Set8bit(!M)
        LDA.L !day
        CMP.B !DAY_STARRY_NIGHT
        BNE .notstarnight
        %Set8bit(!M)
        LDA.B !WEATHER_STARRY_NIGHT         ;winter day 23 fixed festival climate
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .notstarnight:
        ...
        BNE .notwinter
        %Set8bit(!M)
        LDA.B !WEATHER_NEW_YEAR             ;winter day 30 fixed festival climate
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .thunder:
        ...
        %Set8bit(!M)
        LDA.L !day
        CMP.B !DAY_FORCED_THUNDER
        BEQ .Bank82_MainLogicBranch_828E16
        LDA.B #$00
        XBA
        LDA.L !season
        %Set16bit(!M)
    ...
    .Bank82_MainLogicBranch_828E16:
        %Set8bit(!M)
        LDA.B !WEATHER_THUNDER_CALM
        STA.W !weather_tomorrow
        JMP.W .return
    ...
    .ohohhurracane:
        ...
        %Set8bit(!M)
        BNE .normalday
        LDA.B !WEATHER_HURRICANE              ;hurricane / typhoon climate
        STA.W !weather_tomorrow
        BRA .return
...
Hurricane_Chance_Table: db $00,$1E,$00,$00 ;828EB2 ;spring,summer,fall,winter RNG upper bound
Rain_Chance_Table:      db $06,$0A,$0A,$00 ;828EB6 ;spring,summer,fall,winter RNG upper bound
Snow_Chance_Table:      db $00,$00,$00,$03 ;828EBA ;spring,summer,fall,winter RNG upper bound
Thunder_Chance_Table:   db $00,$1E,$00,$00 ;828EBE ;first-year summer thunder chance
Snowstorm_Chance_Table: db $00,$00,$00,$08 ;828EC2 ;first-year winter snowstorm/fair branch chance

;;;;;;;; PASS20_ROMANCE_CORE: writes the name buffer for the girl with the highest heart value.
Romance_UpdateMostLovedGirlNameBuffer: ;828EC6
        %Set16bit(!MX)
        LDY.W #$0000
        LDA.L !hearts_maria
        STA.B $7E
...
;;;;;;;;
AudioTool_PlayToolUseSound1: ;828FB1
        %Set8bit(!M)                             ;      ;
        LDA.B #$00                           ;828FB3;      ;
        XBA                                  ;828FB5;      ;
        ...
        TAX                                  ;828FD1;      ;
        %Set8bit(!M)                             ;828FD2;      ;
        LDA.L ToolAction_UseSoundTable,X                 ;828FD4;829054;
        ...
        TAX                                  ;828FE2;      ;
        %Set8bit(!M)                             ;828FE3;      ;
        LDA.L ToolAction_UseSoundTable,X                 ;828FE5;829054;
        BEQ .return                      ;828FE9;828FF2;
        STA.W $0114                          ;828FEB;000114;
        JSL.L AudioSFX_PlayQueuedEffectDefaultVolume                    ;828FEE;838332;

        .return: RTL                                  ;828FF2;      ;END_AudioTool_PlayToolUseSound1

;;;;;;;;
AudioTool_PlayToolUseSoundIfEnabled: ;828FF3
        %Set8bit(!M)
        LDA.B #$00
        XBA
        LDA.W !tool_selected
        CMP.W $0119
        BEQ .Bank82_MainLogicBranch_82904B
        %Set16bit(!M)
        STA.B $7E
        ASL A
        CLC
        ...
        TAX
        %Set8bit(!M)
        LDA.L ToolAction_UseSoundTable,X
        BEQ .Bank82_MainLogicBranch_82904B
        STA.W $0115
        INX
        PHX
        LDA.L ToolAction_UseSoundTable,X
        BEQ .Bank82_MainLogicBranch_829038
        STA.W $0114
        %Set8bit(!M)
        LDA.B #$00
        XBA
...
;;;;;;;;
ToolAction_UseSoundTable: ;829054
...
;;;;;;;; PASS16: Tool action entry point.
; Called when the player confirms a selected tool/item.
; If no-stamina flag is set, branches to exhausted action. Otherwise it:
;   1) calculates the target tile and stores the tool-use sound result,
;   2) dispatches the visual animation through ToolAction_AnimationJumpTable.
ToolAction_StartAnimationOrNoStamina: ;8290A8
        %Set8bit(!M)
        %Set16bit(!X)
        %Set16bit(!MX)
        LDA.B !game_state
    ...
    .toolused:
        JSR.W ToolAction_PrecalculateTargetTileAndSound
        %Set8bit(!M)
        ...
        ASL A
        TAX
        JSR.W (ToolAction_AnimationJumpTable,X)
        BRA .return
...
;;;;;;;;
ToolAction_ExecuteSelectedToolEffect: ;829260
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.B #$00
        XBA
        ...
        ASL A
        TAX
        JSR.W (ToolAction_EffectJumpTable,X)
        %Set16bit(!M)
        LDA.L $7F1F5A
        ORA.W #$0040                         ;FLAG5A
        STA.L $7F1F5A
...
;;;;;;;;
;;;;;;;; PASS16: Calculates tile in front of the player and asks ToolAction_DispatchTileEffectByHeldTool what this tool can do there.
; Result is also cached in !tool_used_sound for the sound/animation side.
ToolAction_PrecalculateTargetTileAndSound: ;82927D
        %Set16bit(!MX)                             ;      ;
        LDA.W #$0001                         ;82927F;      ;
        LDX.W #$0000                         ;829282;      ;
        LDY.W #$0000                         ;829285;      ;
        JSL.L PlayerTarget_CalculateTileInFront                    ;829288;81D14E;
        %Set16bit(!M)                             ;82928C;      ;
        LDX.W !tile_in_front_X                          ;82928E;000985;
        LDY.W !tile_in_front_Y                          ;829291;000987;
        JSL.L ToolAction_DispatchTileEffectByHeldTool                          ;829294;82AA71;
        %Set8bit(!M)                             ;829298;      ;
        %Set16bit(!X)                             ;82929A;      ;
...
;;;;;;;;
;;;;;;;; PASS16: Check one tile in front of the player.
ToolAction_CheckTargetTileSingle: ;8292A0
        %Set16bit(!MX)
        LDA.W #$0001
        LDX.W #$0000
        LDY.W #$0000
        ...
        JSL.L ToolAction_DispatchTileEffectByHeldTool

        RTS

;;;;;;;;
;;;;;;;; PASS16: Check the next tile in a line pattern. Used by Gold Hoe.
ToolAction_CheckTargetTileLine: ;8292BC
        %Set16bit(!MX)                             ;      ;
        INC A                                ;8292BE;      ;
        LDX.W #$0000                         ;8292BF;      ;
        LDY.W #$0000                         ;8292C2;      ;
        JSL.L PlayerTarget_CalculateTileInFront                    ;8292C5;81D14E;
        %Set16bit(!M)                             ;8292C9;      ;
        LDX.W !tile_in_front_X                          ;8292CB;000985;
        LDY.W !tile_in_front_Y                          ;8292CE;000987;
        JSL.L ToolAction_DispatchTileEffectByHeldTool                          ;8292D1;82AA71;

        RTS                                  ;8292D5;      ;

;;;;;;;;
;;;;;;;; PASS16: Check the next tile in a 3x3 square pattern using ToolSuccessSquareOffsetsTable.
; Used by Gold Sickle, crop/grass seeds and Sprinkler.
ToolAction_CheckTargetTileSquare: ;8292D6
        %Set16bit(!MX)                             ;      ;
        ASL A                                ;8292D8;      ;
        ASL A                                ;8292D9;      ;
        TAX                                  ;8292DA;      ;
        ...
        PLA                                  ;8292E7;      ;
        TAX                                  ;8292E8;      ;
        JSL.L Bank82_MainLogicBranch_81D1A4                    ;8292E9;81D1A4;
        %Set16bit(!M)                             ;8292ED;      ;
        LDX.W !tile_in_front_X                          ;8292EF;000985;
        LDY.W !tile_in_front_Y                          ;8292F2;000987;
        JSL.L ToolAction_DispatchTileEffectByHeldTool                          ;8292F5;82AA71;
...
;;;;;;;;
AnimalFeed_StallBitmaskTable: ;82A571
        dw $0001,$0002,$0004,$0008,$0010,$0020,$0040,$0080
        dw $0100,$0200,$0400,$0800,$1000

;;;;;;;; PASS16: tool_selected -> animation routine.
ToolAction_AnimationJumpTable: ;82A58B
...
;;;;;;;; PASS16: tool_selected -> gameplay effect routine.
ToolAction_EffectJumpTable: ;82A5C3
        dw ToolUsedNoAction
        dw ToolUsedSickle
        dw ToolUsedHoe
        dw ToolUsedHammer
...
LoadMap: ;82A5FB
    ...
    .set0D:
        %Set16bit(!MX)
        PLX
        LDA.L TileProperty_RulesetPointerTable,X            ;
        STA.B $0D
        INX
        INX
        %Set8bit(!M)
...
;;;;;;;;
;;;;;;;; PASS17: Winter/monthly farm cleanup.
;;;;;;;; Runs on Winter day 1 through the nightly farm pipeline.
;;;;;;;; Converts crop/grass ranges into winter-safe field states without changing map size.
FarmTiles_WinterMonthlyCleanup: ;82A6A2
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.L !season
        CMP.B #$03                           ;winter
    ...
    .outerloop:
        ...
        .notoutofbound:
            CMP.B #$70
            BCS .Bank82_MainLogicBranch_82A6E0
            CMP.B #$1D
            BCS .Bank82_MainLogicBranch_82A6EA
            BRA .skiptile

        .Bank82_MainLogicBranch_82A6E0:
            %Set8bit(!M)
            LDA.B #$7A
            STA.L !farm_map_array,X
            BRA .skiptile

        .Bank82_MainLogicBranch_82A6EA:
            %Set8bit(!M)
            LDA.B #$07
            STA.L !farm_map_array,X
...
;;;;;;;; PASS17: Applies climate damage to farm map tiles.
;;;;;;;; Parameters: A=fence chance, X=grass chance, Y=crop/tilled-soil chance.
;;;;;;;; Damages fences, grass and crop/tilled soil by editing !farm_map_array in place.
FarmTiles_ApplyClimateDamage: ;82A713
        %Set16bit(!MX)
        STA.B $86
        STX.B $88
        STY.B $8A
...
;;;;;;;;
;;;;;;;; PASS17: Daily crop/field lifecycle core.
;;;;;;;; Handles rain watering, dry plots, crop/grass growth, random debris and broken-fence flags.
;;;;;;;; This is the main routine behind overnight plant/tile progression.
FarmTiles_NightlyGrowthDryingAndCleanup: ;82A811
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.B #$04
        STA.W $0181
    ...
    .outerloop:
        ...
        .notgrass:
            CMP.B #$20
            BCS .Bank82_MainLogicBranch_82A86F
            JMP.W .skiploop

        .Bank82_MainLogicBranch_82A86F:
            CMP.B #$39                       ;watered fully grown Tomato
            BNE .Bank82_MainLogicBranch_82A876
            JMP.W .unwateredraincheck

        .Bank82_MainLogicBranch_82A876:
            CMP.B #$53                       ;watered fully grown Corn
            BNE .Bank82_MainLogicBranch_82A87D
            JMP.W .unwateredraincheck

        .Bank82_MainLogicBranch_82A87D:
            CMP.B #$61                       ;watered fully grown Potato
            BNE .Bank82_MainLogicBranch_82A884
            JMP.W .unwateredraincheck

        .Bank82_MainLogicBranch_82A884:
            CMP.B #$6F                       ;watered fully grown Turnip
            BNE .Bank82_MainLogicBranch_82A88B
            JMP.W .unwateredraincheck

        .Bank82_MainLogicBranch_82A88B:
            CMP.B #$79                       ;grass grass third stage
            BNE .Bank82_MainLogicBranch_82A892
            JMP.W .skiploop

        .Bank82_MainLogicBranch_82A892:
            CMP.B #$7C                       ;grass second stage
            BNE .Bank82_MainLogicBranch_82A899
            JMP.W .Bank82_MainLogicBranch_82A8CF

        .Bank82_MainLogicBranch_82A899:
            CMP.B #$70                       ;grass first stage+
            BCS .watertile
            AND.B #$01                       ;???
            BEQ .wateredraincheck
            JMP.W .Bank82_MainLogicBranch_82A8DA
        ...
        .addrandomtrash:
            ...
            %Set16bit(!X)                             ;82A931;      ;
            PHX                                  ;82A933;      ;
            JSL.L RNG_GetNextByte                         ;82A934;838138;
            %Set8bit(!M)                             ;82A938;      ;
            %Set16bit(!X)                             ;82A93A;      ;
            PLX                                  ;82A93C;      ;
            CMP.B #$00                           ;82A93D;      ;
        ...
        .brokenfence:
            %Set8bit(!M)                             ;82A949;      ;
            LDA.B #$08                           ;82A94B;      ;
            JSL.L RNG_GetRange0ToAExclusiveStyle                  ;82A94D;8089F9;
            BNE .skiploop                      ;82A951;82A969;
            %Set16bit(!M)                             ;82A953;      ;
            LDA.W $0196                          ;82A955;000196;
            ORA.W #$0400                         ;82A958;      ;
            STA.W $0196                          ;82A95B;000196;
            LDA.L !child_flags                        ;82A95E;7F1F6E;
            ORA.W #$0200                         ;82A962;      ;
            STA.L !child_flags                        ;82A965;7F1F6E;
        ...
        .skiploop:
            ...
            TAX                                  ;82A972;      ;
            CPX.W #$0400                         ;82A973;      ;
            BEQ .Bank82_MainLogicBranch_82A97B                      ;82A976;82A97B;
            JMP.W .innerloop                    ;82A978;82A822;

    .Bank82_MainLogicBranch_82A97B:
        TYA                                  ;82A97B;      ;
        CLC                                  ;82A97C;      ;
        ADC.W #$0010                         ;82A97D;      ;
        TAY                                  ;82A980;      ;
        CPY.W #$0400                         ;82A981;      ;
        BEQ .Bank82_MainLogicBranch_82A989                      ;82A984;82A989;
        JMP.W .outerloop                    ;82A986;82A81F;

    .Bank82_MainLogicBranch_82A989:
        %Set8bit(!M)
        LDA.L !season
        CMP.B #$03
        BNE .return
        ...
        CMP.B #$01
        BNE .return
        JSL.L FarmTiles_WinterMonthlyCleanup
...
;;;;;;;;
;;;;;;;; PASS17: Counts productive/developed farm tiles.
;;;;;;;; Fences, broken fences, grass/crops and used field tiles increase !ranch_development.
Farm_CalculateRanchDevelopmentScore: ;82AA0C
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.B #$04
        STA.W $0181
...
;;;;;;;; PASS17: Tool target validator.
;;;;;;;; Checks whether the selected tool is allowed to affect the target tile.
;;;;;;;; Returns 0=fail, 1=success, 2=special watering-can fill path.
ToolAction_DispatchTileEffectByHeldTool: ;82AA71
        %Set8bit(!M)
        %Set16bit(!X)
        LDA.W !tool_selected
        CMP.B #$01                           ;sickle
...
;;;;;;;; PASS17: Directional tile-property checker.
;;;;;;;; Reads tile content/properties and applies direction masks used by tool/crop interactions.
TileProperty_CheckToolUseAllowed: ;82AC61
;;;;;;;; PASS52: Core tile-property rule checker.
;;;;;;;; Uses the ruleset pointer installed by LoadMap in $0D-$0F.
;;;;;;;; Layout per tile ID: 4 words/8 bytes. Offset used here is tile_id * 4 + 3,
;;;;;;;; which selects the direction/season permission byte for tool interaction gates.
        %Set16bit(!MX)
        STA.B $82
        %Set16bit(!M)
        LDA.L $7F1F5C
    ...
    .continue:
        ...
        ASL A
        TAX
        LDA.L TileProperty_DirectionMaskTable,X
        STA.B $84
        %Set8bit(!M)
        LDA.B #$00
        XBA
        ...
        ASL A
        TAX
        LDA.L TileProperty_SeasonMaskTable,X
        STA.B $84
        %Set8bit(!M)
        LDA.B #$00
        XBA
...
TileProperty_DirectionMaskTable: db $01,$00,$02,$00,$04,$00,$08,$00   ;82ACFE;      ;
;;;;;;;; PASS52: Direction mask order follows player direction words.
;;;;;;;; Bit meaning used by TileProperty_CheckToolUseAllowed: down/up/left/right permission.
TileProperty_SeasonMaskTable: db $10,$00,$20,$00,$40,$00,$80,$00   ;82AD06;      ;
;;;;;;;; PASS52: Season mask order follows season index words.
;;;;;;;; Bits $10/$20/$40/$80 gate tile interactions by Spring/Summer/Fall/Winter.

;;;;;;;;
PlayerInteraction_DispatchFacingTileAction: ;82AD0E
        %Set8bit(!M)
        %Set16bit(!X)
        CPX.W #$00F0
        BCC .Bank82_MainLogicBranch_82AD1A
        JMP.W .Bank82_MainLogicBranch_82AEBA

    .Bank82_MainLogicBranch_82AD1A:
        CPX.W #$00E0
        BCC .Bank82_MainLogicBranch_82AD22
        JMP.W .Bank82_MainLogicBranch_82AEB8

    .Bank82_MainLogicBranch_82AD22:
        CPX.W #$00D0
        BCC .Bank82_MainLogicBranch_82AD2A
        JMP.W .Bank82_MainLogicBranch_82AE9B

    .Bank82_MainLogicBranch_82AD2A:
        CPX.W #$00C0
        BCC .Bank82_MainLogicBranch_82AD32
        JMP.W .Bank82_MainLogicBranch_82ADE0

    .Bank82_MainLogicBranch_82AD32:
        %Set8bit(!M)
        LDA.L !season
        CMP.B #$02
        BEQ .Bank82_MainLogicBranch_82AD42
        CMP.B #$03
        BEQ .Bank82_MainLogicBranch_82AD42
        BRA .Bank82_MainLogicBranch_82AD84

    .Bank82_MainLogicBranch_82AD42:
        %Set16bit(!MX)
        CPX.W #$0038
        BNE .Bank82_MainLogicBranch_82AD4C
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD4C:
        CPX.W #$0039
        BNE .Bank82_MainLogicBranch_82AD54
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD54:
        CPX.W #$0052
        BNE .Bank82_MainLogicBranch_82AD5C
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD5C:
        CPX.W #$0053
        BNE .Bank82_MainLogicBranch_82AD64
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD64:
        CPX.W #$0060
        BNE .Bank82_MainLogicBranch_82AD6C
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD6C:
        CPX.W #$0061
        BNE .Bank82_MainLogicBranch_82AD74
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD74:
        CPX.W #$006E
        BNE .Bank82_MainLogicBranch_82AD7C
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD7C:
        CPX.W #$006F
        BNE .Bank82_MainLogicBranch_82AD84
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AD84:
        %Set16bit(!M)
        TXA
        ASL A
        ASL A
        ...
        LDA.B [$0D],Y
        AND.B #$80
        BNE .Bank82_MainLogicBranch_82AD96
        JMP.W .Bank82_MainLogicBranch_82ADC5

    .Bank82_MainLogicBranch_82AD96:
        DEY
        LDA.B [$0D],Y
        STA.W $091F
        %Set16bit(!M)
        ...
        TAX
        %Set8bit(!M)
        LDA.L MapObject_PickupItemIdTable,X
        STA.W !item_on_hand
        %Set16bit(!MX)
        LDA.W #$0004
        STA.B !player_action
    ...
    .Bank82_MainLogicBranch_82ADC5:
        %Set8bit(!M)
        INY
        LDA.B [$0D],Y
        AND.B #$01
        BNE .Bank82_MainLogicBranch_82ADD1
        JMP.W .Bank82_MainLogicBranch_82AEE6

    .Bank82_MainLogicBranch_82ADD1:
        %Set16bit(!MX)
        LDA.B !game_state
        AND.W #$0020
        BEQ .Bank82_MainLogicBranch_82ADDD
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82ADDD:
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82ADE0:
        %Set16bit(!M)
        TXA
        ASL A
        ASL A
        ...
        LDA.B [$0D],Y
        STA.B $93
        BNE .Bank82_MainLogicBranch_82ADF9
        JMP.W .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82ADF9:
        LDA.B #$00
        XBA
        LDA.B $92
        %Set16bit(!M)
        ...
        LDA.L Unk_Table13,X
        AND.B #$02
        BEQ .Bank82_MainLogicBranch_82AE1B
        LDA.L !hour
        CMP.B #$11
        BCC .Bank82_MainLogicBranch_82AE1B
        BRA .Bank82_MainLogicBranch_82AE81

    .Bank82_MainLogicBranch_82AE1B:
        %Set8bit(!M)
        LDA.L Unk_Table13,X
        AND.B #$04
        BEQ .Bank82_MainLogicBranch_82AE2F
        LDA.L !hour
        CMP.B #$11
        BCS .Bank82_MainLogicBranch_82AE2F
        BRA .Bank82_MainLogicBranch_82AE81

    .Bank82_MainLogicBranch_82AE2F:
        %Set8bit(!M)
        LDA.L Unk_Table13,X
        AND.B #$08
        BEQ .Bank82_MainLogicBranch_82AE45
        LDA.L !weekday
        BEQ .Bank82_MainLogicBranch_82AE45
        CMP.B #$06
        BEQ .Bank82_MainLogicBranch_82AE45
    ...
    .Bank82_MainLogicBranch_82AE45:
        %Set8bit(!M)
        LDA.L Unk_Table13,X
        AND.B #$10
        BEQ .Bank82_MainLogicBranch_82AE59
        LDA.L !weekday
        CMP.B #$06
        BNE .Bank82_MainLogicBranch_82AE59
        BRA .Bank82_MainLogicBranch_82AE81

    .Bank82_MainLogicBranch_82AE59:
        %Set8bit(!M)
        LDA.L Unk_Table13,X
        AND.B #$20
        BEQ .Bank82_MainLogicBranch_82AE6B
        LDA.L !weekday
        BNE .Bank82_MainLogicBranch_82AE6B
        BRA .Bank82_MainLogicBranch_82AE81

    .Bank82_MainLogicBranch_82AE6B:
        %Set8bit(!M)
        LDA.L Unk_Table13,X
        AND.B #$80
        BEQ .Bank82_MainLogicBranch_82AEE5
        %Set16bit(!M)
        LDA.W $0196
        AND.W #$0010
        BEQ .Bank82_MainLogicBranch_82AEE5
    ...
    .Bank82_MainLogicBranch_82AE81:
        ...
        BRA .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AE9B:
        %Set16bit(!M)
        TXA
        ASL A
        ASL A
    ...
    .Bank82_MainLogicBranch_82AEB8:
        BRA .Bank82_MainLogicBranch_82AEE5

    .Bank82_MainLogicBranch_82AEBA:
        %Set16bit(!M)
        TXA
        ASL A
        ASL A
    ...
    .Bank82_MainLogicBranch_82AEE5:
        RTL

    .Bank82_MainLogicBranch_82AEE6:
        %Set16bit(!M)
        LDA.B !player_pos_X
        STA.B $DF
        LDA.B !player_pos_Y
        ...
        STA.B $E3
        LDA.B !player_direction
        JSL.L Collision_CheckMapTileAtDirection

        RTL

;;;;;;;;
FarmMap_ApplyPersistentFarmTileOverlay: ;82AF00
        %Set16bit(!MX)
        LDY.W #$0000
    ...
    .bigloop:
        ...
        .skip8:
            ...
            TAX                                  ;82AFC1;      ;
            %Set8bit(!M)                             ;82AFC2;      ;
            LDA.L FarmTile_NeighborVariantOffsetTable,X                 ;82AFC4;82B02A;
            DEC A                                ;82AFC8;      ;
        ...
        .skip9:
            ...
            JSL.L MapTilePatch_CopyTileObjectToWRAM                           ;82B005;81A83A;
    ...
    .return: RTL
                                                            ;      ;      ;
                                                            ;      ;      ;
FarmTile_NeighborVariantOffsetTable: db $09,$02,$07,$01,$07,$04,$07,$04,$09,$03,$08,$02,$09,$06,$08,$05;82B02A;      ;
...
;;;;;;;; Fills a rectangular block in !current_map_array.
;;;;;;;; A=tile id, X/Y=start tile position, $86=width, $88=height.
MapTile_FillRectCurrentMap: ;82B060
        %Set8bit(!M)
        %Set16bit(!X)
        STA.B $99
        STX.B $82
...
;;;;;;;; Applies a compressed/encoded map placement record to !current_map_array.
;;;;;;;; Used by scripted map-entry/object visual patch routines.
MapTilePatch_ApplyPlacementRecord: ;82B0A7
        %Set16bit(!MX)
        STX.B $8E
        STY.B $90
...
TileProperty_RulesetPointerTable:  dl TilePropertyRuleset_00_FarmSeasonalOutdoor
;;;;;;;; PASS52: Parallel to MapPointerTable. LoadMap picks a map data pointer and the
;;;;;;;; matching tile-property ruleset pointer for collision/tool/tile checks.
                    dl TilePropertyRuleset_00_FarmSeasonalOutdoor
                    dl TilePropertyRuleset_00_FarmSeasonalOutdoor
                    dl TilePropertyRuleset_00_FarmSeasonalOutdoor
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    ...
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_03_GeneralOutdoorInterior
                    dl TilePropertyRuleset_02_TownOutdoorObjectRows
                    dl TilePropertyRuleset_02_TownOutdoorObjectRows
                    dl TilePropertyRuleset_02_TownOutdoorObjectRows
                    dl TilePropertyRuleset_02_TownOutdoorObjectRows
                    dl TilePropertyRuleset_04_MountainForestOutdoor
                    dl TilePropertyRuleset_04_MountainForestOutdoor
                    dl TilePropertyRuleset_04_MountainForestOutdoor
                    dl TilePropertyRuleset_04_MountainForestOutdoor
                    dl TilePropertyRuleset_04_MountainForestOutdoor
                    dl TilePropertyRuleset_01_SparseInterior
                    dl TilePropertyRuleset_01_SparseInterior
                    dl TilePropertyRuleset_01_SparseInterior
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_05_InteriorSpecialA
                    dl TilePropertyRuleset_06_InteriorSpecialB
                    dl TilePropertyRuleset_06_InteriorSpecialB
                    dl TilePropertyRuleset_06_InteriorSpecialB
                    dl TilePropertyRuleset_06_InteriorSpecialB
                    dl TilePropertyRuleset_06_InteriorSpecialB
...
TilePropertyRuleset_00_FarmSeasonalOutdoor:  dw $0000,$FF00,$0200,$FF00,$020E,$FF00,$810F,$7F01;82B3B4;      ;
...
TilePropertyRuleset_01_SparseInterior:  dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82B7B4;      ;
                ...
                dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82BBA4;      ;
                                                            ;      ;      ;
TilePropertyRuleset_02_TownOutdoorObjectRows:  dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82BBB4;      ;
...
TilePropertyRuleset_03_GeneralOutdoorInterior:  dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82BFB4;      ;
...
TilePropertyRuleset_04_MountainForestOutdoor:  dw $0000,$FF00,$00DE,$FF00,$00DD,$FF00,$009C,$FF00;82C3B4;      ;
...
TilePropertyRuleset_05_InteriorSpecialA:  dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82C7B4;      ;
...
TilePropertyRuleset_06_InteriorSpecialB:  dw $0000,$FF00,$0000,$FF00,$0000,$FF00,$0000,$FF00;82CBB4;      ;
...
MapObject_PickupItemIdTable:   db $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$09,$09,$0A,$00;82CFB4;      ;
...
;;;;;;;;
NewGame_ResetSaveStateAndEnterFarmLoop: ;82D1C0
        %Set8bit(!M)                             ;82D1C0;      ;
        %Set16bit(!X)                             ;82D1C2;      ;
        LDA.B #$0F                           ;82D1C4;      ;
        ...
        JSL.L VideoFade_Out                  ;82D1D0;80880A;
        %Set8bit(!M)                             ;82D1D4;      ;
        LDA.B #$06                           ;82D1D6;      ;
        ...
        %Set16bit(!M)                             ;82D222;      ;
        LDA.W #$0000                         ;82D224;      ;
        STA.L !player_house_and_event_flags                        ;82D227;7F1F64;
        LDA.W #$0000                         ;82D22B;      ;
        STA.L !marriage_flags                        ;82D22E;7F1F66;
        LDA.W #$0000                         ;82D232;      ;
        STA.L $7F1F68                        ;82D235;7F1F68;
        LDA.W #$0000                         ;82D239;      ;
        STA.L $7F1F6A                        ;82D23C;7F1F6A;
        LDA.W #$0000                         ;82D240;      ;
        STA.L !family_event_flags                        ;82D243;7F1F6C;
        LDA.W #$0000                         ;82D247;      ;
        STA.L !child_flags                        ;82D24A;7F1F6E;
        LDA.W #$0000                         ;82D24E;      ;
        ...
        ; PASS40_FIELD_STATUS_MENU_HANDOFF: clear any pending field/status
        ; menu request before this field/new-game loop starts.
        LDA.L $7F1F5C                        ;82D2E8;7F1F5C;
        AND.W #$FFFB                         ;82D2EC;      ;
        STA.L $7F1F5C                        ;82D2EF;7F1F5C;
        JSL.L VRAM_ClearAll                      ;82D2F3;808846;
        JSL.L Palette_CGRAM_ClearAll                     ;82D2F7;808980;
        JSL.L PaletteAnim_ClearAllPointer42Slots           ;82D2FB;808FAB;
        JSL.L ClearSpriteDataTables         ;82D2FF;858ED7;
        JSL.L InitializeOBJs         ;82D303;85820F;
        JSL.L MapTilePatchRuntime_ClearAllSlots                 ;82D307;81A4C7;
        JSL.L EventScript_ClearAllSlots                 ;82D30B;848000;
        ...
        LDX.W #$0047                         ;82D320;      ;
        LDY.W #$0000                         ;82D323;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D326;848097;
        JSL.L EventScript_UpdateAllActiveSlots                    ;82D32A;84816F;
        %Set8bit(!M)                             ;82D32E;      ;
        LDA.W !transition_dest                          ;82D330;00098B;
        STA.B !tilemap_to_load                            ;82D333;000022;
        JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82D335;8095DE;
        %Set8bit(!M)                             ;82D339;      ;
        LDA.W !transition_dest                          ;82D33B;00098B;
        JSL.L MapLoad_LoadAreaByIdAndApplyOverlays                           ;82D33E;80972C;
        %Set16bit(!MX)                             ;82D342;      ;
        LDY.W #$0000                         ;82D344;      ;
    ...
    .waitforNMI:
        ...
        JSL.L MapTransition_BeginFadeAndLoadDestination         ;82D358;809671;
        JSL.L MapTransition_CheckEdgeTriggerAndQueueDestination                           ;82D35C;809A64;
        JSL.L Clock_UpdateAndDispatchHourlyEvents                     ;82D360;828000;
        JSL.L TextBox_UpdateRendererAndControlCodes                      ;82D364;83951C;
        JSL.L PaletteTransition_StepTimeOfDayFade           ;82D368;80900C;
        JSL.L PaletteAnim_UpdatePointer42Slots                    ;82D36C;808E69;
        JSL.L Input_DispatchByState                          ;82D370;84C034;
        JSL.L MapTilePatchRuntime_UpdateActiveSlots                          ;82D374;81A383;
        JSL.L PlayerAction_DispatchAndUpdateCamera                          ;82D378;81BFB7;
        JSL.L AutoMapScrolling               ;82D37C;8095B3;
        JSL.L EventScript_UpdateAllActiveSlots                    ;82D380;84816F;
        JSL.L MapTilePatchScript_UpdateActive                    ;82D384;81A600;
        JSL.L GameOBJ_UpdateAnimationFrames                    ;82D388;8582C7;
        JSL.L SpriteComponent_BuildVRAMUploadQueue                    ;82D38C;858CB2;
        JSL.L PrepareOAMData             ;82D390;8583E0;
        %Set16bit(!M)                             ;82D394;      ;
        ; PASS40_FIELD_STATUS_MENU_HANDOFF: Start/status input sets
        ; $7F1F5C bit $0004 in bank 84. The field loop consumes it by
        ; reentering the intro/menu dispatcher with $95 = $09.
        LDA.L $7F1F5C                        ;82D396;7F1F5C;
        AND.W #$0004                         ;82D39A;      ;
        BNE .fieldStatusMenuRequestPending                      ;82D39D;82D3A6;
        %Set8bit(!M)                             ;82D39F;      ;
        STZ.B !NMI_Status                            ;82D3A1;000000;
        JMP.W .waitforNMI                    ;82D3A3;82D347;

    .fieldStatusMenuRequestPending:
        %Set16bit(!MX)                             ;82D3A6;      ;
        LDX.W #$0000                         ;82D3A8;      ;

    .clearFieldMenuScratch:
        %Set8bit(!M)                             ;82D3AB;      ;
        LDA.B #$00                           ;82D3AD;      ;
        STA.L !chicken_array,X                      ;82D3AF;7EC286;
        INX                                  ;82D3B3;      ;
        CPX.W #$0008                         ;82D3B4;      ;
        BNE .clearFieldMenuScratch                      ;82D3B7;82D3AB;
...
DemoPresentation:
        ...
        JSL.L VideoFade_Out                  ;82D3D7;80880A;
        %Set8bit(!M)                             ;82D3DB;      ;
        LDA.B #$06                           ;82D3DD;      ;
        ...
        %Set16bit(!M)                             ;82D429;      ;
        LDA.W #$0000                         ;82D42B;      ;
        STA.L !player_house_and_event_flags                        ;82D42E;7F1F64;
        LDA.W #$0000                         ;82D432;      ;
        STA.L !marriage_flags                        ;82D435;7F1F66;
        LDA.W #$0000                         ;82D439;      ;
        STA.L $7F1F68                        ;82D43C;7F1F68;
        LDA.W #$0000                         ;82D440;      ;
        STA.L $7F1F6A                        ;82D443;7F1F6A;
        LDA.W #$0000                         ;82D447;      ;
        STA.L !family_event_flags                        ;82D44A;7F1F6C;
        LDA.W #$0000                         ;82D44E;      ;
        STA.L !child_flags                        ;82D451;7F1F6E;
        LDA.W #$0000                         ;82D455;      ;
        ...
        JSL.L VRAM_ClearAll                      ;82D4F4;808846;
        JSL.L Palette_CGRAM_ClearAll                     ;82D4F8;808980;
        JSL.L PaletteAnim_ClearAllPointer42Slots           ;82D4FC;808FAB;
        JSL.L ClearSpriteDataTables         ;82D500;858ED7;
        JSL.L InitializeOBJs         ;82D504;85820F;
        JSL.L MapTilePatchRuntime_ClearAllSlots                 ;82D508;81A4C7;
        JSL.L EventScript_ClearAllSlots                 ;82D50C;848000;
        ...
        BNE .Bank82_MainLogicBranch_82D527                      ;82D522;82D527;
        JMP.W .Bank82_MainLogicBranch_82D574                    ;82D524;82D574;

    .Bank82_MainLogicBranch_82D527:
        CMP.B #$01                           ;82D527;      ;
        BNE .Bank82_MainLogicBranch_82D52E                      ;82D529;82D52E;
        JMP.W .Bank82_MainLogicBranch_82D58E                    ;82D52B;82D58E;

    .Bank82_MainLogicBranch_82D52E:
        CMP.B #$02                           ;82D52E;      ;
        BNE .Bank82_MainLogicBranch_82D535                      ;82D530;82D535;
        JMP.W .Bank82_MainLogicBranch_82D5A8                    ;82D532;82D5A8;

    .Bank82_MainLogicBranch_82D535:
        CMP.B #$03                           ;82D535;      ;
        BNE .Bank82_MainLogicBranch_82D53C                      ;82D537;82D53C;
        JMP.W .Bank82_MainLogicBranch_82D5C2                    ;82D539;82D5C2;

    .Bank82_MainLogicBranch_82D53C:
        CMP.B #$04                           ;82D53C;      ;
        BNE .Bank82_MainLogicBranch_82D543                      ;82D53E;82D543;
        JMP.W .Bank82_MainLogicBranch_82D5DC                    ;82D540;82D5DC;

    .Bank82_MainLogicBranch_82D543:
        CMP.B #$05                           ;82D543;      ;
        BNE .Bank82_MainLogicBranch_82D54A                      ;82D545;82D54A;
        JMP.W .Bank82_MainLogicBranch_82D5F6                    ;82D547;82D5F6;

    .Bank82_MainLogicBranch_82D54A:
        CMP.B #$06                           ;82D54A;      ;
        BNE .Bank82_MainLogicBranch_82D551                      ;82D54C;82D551;
        JMP.W .Bank82_MainLogicBranch_82D610                    ;82D54E;82D610;

    .Bank82_MainLogicBranch_82D551:
        CMP.B #$07                           ;82D551;      ;
        BNE .Bank82_MainLogicBranch_82D558                      ;82D553;82D558;
        JMP.W .Bank82_MainLogicBranch_82D62A                    ;82D555;82D62A;

    .Bank82_MainLogicBranch_82D558:
        CMP.B #$08                           ;82D558;      ;
        BNE .Bank82_MainLogicBranch_82D55F                      ;82D55A;82D55F;
        JMP.W .Bank82_MainLogicBranch_82D644                    ;82D55C;82D644;

    .Bank82_MainLogicBranch_82D55F:
        CMP.B #$09                           ;82D55F;      ;
        BNE .Bank82_MainLogicBranch_82D566                      ;82D561;82D566;
        JMP.W .Bank82_MainLogicBranch_82D65E                    ;82D563;82D65E;

    .Bank82_MainLogicBranch_82D566:
        CMP.B #$0A                           ;82D566;      ;
        BNE .Bank82_MainLogicBranch_82D56D                      ;82D568;82D56D;
        JMP.W .Bank82_MainLogicBranch_82D678                    ;82D56A;82D678;

    .Bank82_MainLogicBranch_82D56D:
        CMP.B #$0B                           ;82D56D;      ;
        BNE .Bank82_MainLogicBranch_82D574                      ;82D56F;82D574;
        JMP.W .Bank82_MainLogicBranch_82D692                    ;82D571;82D692;

    .Bank82_MainLogicBranch_82D574:
        %Set16bit(!MX)                             ;82D574;      ;
        LDA.W #$0000                         ;82D576;      ;
        LDX.W #$0047                         ;82D579;      ;
        LDY.W #$0002                         ;82D57C;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D57F;848097;
        %Set8bit(!M)                             ;82D583;      ;
        LDA.B #$01                           ;82D585;      ;
        STA.L $7F1F48                        ;82D587;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D58B;82D6AC;

    .Bank82_MainLogicBranch_82D58E:
        %Set16bit(!MX)                             ;82D58E;      ;
        LDA.W #$0000                         ;82D590;      ;
        LDX.W #$0047                         ;82D593;      ;
        LDY.W #$0004                         ;82D596;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D599;848097;
        %Set8bit(!M)                             ;82D59D;      ;
        LDA.B #$02                           ;82D59F;      ;
        STA.L $7F1F48                        ;82D5A1;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D5A5;82D6AC;

    .Bank82_MainLogicBranch_82D5A8:
        %Set16bit(!MX)                             ;82D5A8;      ;
        LDA.W #$0000                         ;82D5AA;      ;
        LDX.W #$0047                         ;82D5AD;      ;
        LDY.W #$0006                         ;82D5B0;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D5B3;848097;
        %Set8bit(!M)                             ;82D5B7;      ;
        LDA.B #$03                           ;82D5B9;      ;
        STA.L $7F1F48                        ;82D5BB;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D5BF;82D6AC;

    .Bank82_MainLogicBranch_82D5C2:
        %Set16bit(!MX)                             ;82D5C2;      ;
        LDA.W #$0000                         ;82D5C4;      ;
        LDX.W #$0047                         ;82D5C7;      ;
        LDY.W #$0007                         ;82D5CA;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D5CD;848097;
        %Set8bit(!M)                             ;82D5D1;      ;
        LDA.B #$04                           ;82D5D3;      ;
        STA.L $7F1F48                        ;82D5D5;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D5D9;82D6AC;

    .Bank82_MainLogicBranch_82D5DC:
        %Set16bit(!MX)                             ;82D5DC;      ;
        LDA.W #$0000                         ;82D5DE;      ;
        LDX.W #$0047                         ;82D5E1;      ;
        LDY.W #$0009                         ;82D5E4;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D5E7;848097;
        %Set8bit(!M)                             ;82D5EB;      ;
        LDA.B #$05                           ;82D5ED;      ;
        STA.L $7F1F48                        ;82D5EF;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D5F3;82D6AC;

    .Bank82_MainLogicBranch_82D5F6:
        %Set16bit(!MX)                             ;82D5F6;      ;
        LDA.W #$0000                         ;82D5F8;      ;
        LDX.W #$0047                         ;82D5FB;      ;
        LDY.W #$000B                         ;82D5FE;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D601;848097;
        %Set8bit(!M)                             ;82D605;      ;
        LDA.B #$06                           ;82D607;      ;
        STA.L $7F1F48                        ;82D609;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D60D;82D6AC;

    .Bank82_MainLogicBranch_82D610:
        %Set16bit(!MX)                             ;82D610;      ;
        LDA.W #$0000                         ;82D612;      ;
        LDX.W #$0047                         ;82D615;      ;
        LDY.W #$000D                         ;82D618;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D61B;848097;
        %Set8bit(!M)                             ;82D61F;      ;
        LDA.B #$07                           ;82D621;      ;
        STA.L $7F1F48                        ;82D623;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D627;82D6AC;

    .Bank82_MainLogicBranch_82D62A:
        %Set16bit(!MX)                             ;82D62A;      ;
        LDA.W #$0000                         ;82D62C;      ;
        LDX.W #$0047                         ;82D62F;      ;
        LDY.W #$0011                         ;82D632;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D635;848097;
        %Set8bit(!M)                             ;82D639;      ;
        LDA.B #$08                           ;82D63B;      ;
        STA.L $7F1F48                        ;82D63D;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D641;82D6AC;

    .Bank82_MainLogicBranch_82D644:
        %Set16bit(!MX)                             ;82D644;      ;
        LDA.W #$0000                         ;82D646;      ;
        LDX.W #$0047                         ;82D649;      ;
        LDY.W #$0015                         ;82D64C;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D64F;848097;
        %Set8bit(!M)                             ;82D653;      ;
        LDA.B #$09                           ;82D655;      ;
        STA.L $7F1F48                        ;82D657;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D65B;82D6AC;

    .Bank82_MainLogicBranch_82D65E:
        %Set16bit(!MX)                             ;82D65E;      ;
        LDA.W #$0000                         ;82D660;      ;
        LDX.W #$0047                         ;82D663;      ;
        LDY.W #$0017                         ;82D666;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D669;848097;
        %Set8bit(!M)                             ;82D66D;      ;
        LDA.B #$0A                           ;82D66F;      ;
        STA.L $7F1F48                        ;82D671;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D675;82D6AC;

    .Bank82_MainLogicBranch_82D678:
        %Set16bit(!MX)                             ;82D678;      ;
        LDA.W #$0000                         ;82D67A;      ;
        LDX.W #$0047                         ;82D67D;      ;
        LDY.W #$0019                         ;82D680;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D683;848097;
        %Set8bit(!M)                             ;82D687;      ;
        LDA.B #$0B                           ;82D689;      ;
        STA.L $7F1F48                        ;82D68B;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D68F;82D6AC;

    .Bank82_MainLogicBranch_82D692:
        %Set16bit(!MX)                             ;82D692;      ;
        LDA.W #$0000                         ;82D694;      ;
        LDX.W #$0047                         ;82D697;      ;
        LDY.W #$001B                         ;82D69A;      ;
        JSL.L EventScript_LoadScriptPointerLong                            ;82D69D;848097;
        %Set8bit(!M)                             ;82D6A1;      ;
        LDA.B #$00                           ;82D6A3;      ;
        STA.L $7F1F48                        ;82D6A5;7F1F48;
        JMP.W .Bank82_MainLogicBranch_82D6AC                    ;82D6A9;82D6AC;

    .Bank82_MainLogicBranch_82D6AC:
        JSL.L EventScript_UpdateAllActiveSlots                    ;82D6AC;84816F;
        %Set8bit(!M)                             ;82D6B0;      ;
        LDA.W !transition_dest                          ;82D6B2;00098B;
        STA.B !tilemap_to_load                            ;82D6B5;000022;
        JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82D6B7;8095DE;
        %Set8bit(!M)                             ;82D6BB;      ;
        LDA.W !transition_dest                          ;82D6BD;00098B;
        JSL.L MapLoad_LoadAreaByIdAndApplyOverlays                           ;82D6C0;80972C;
        %Set16bit(!MX)                             ;82D6C4;      ;
        LDY.W #$0000                         ;82D6C6;      ;

    .Bank82_MainLogicBranch_82D6C9:
        %Set16bit(!M)                             ;82D6C9;      ;
        STZ.B $90                            ;82D6CB;000090;
        %Set8bit(!M)                             ;82D6CD;      ;
        LDA.B !NMI_Status                            ;82D6CF;000000;
        BEQ .Bank82_MainLogicBranch_82D6C9                      ;82D6D1;82D6C9;
        %Set16bit(!M)                             ;82D6D3;      ;
        LDA.W #$1800                         ;82D6D5;      ;
        STA.B $C7                            ;82D6D8;0000C7;
        JSL.L MapTransition_BeginFadeAndLoadDestination         ;82D6DA;809671;
        JSL.L MapTransition_CheckEdgeTriggerAndQueueDestination                           ;82D6DE;809A64;
        JSL.L Clock_UpdateAndDispatchHourlyEvents                     ;82D6E2;828000;
        JSL.L TextBox_UpdateRendererAndControlCodes                      ;82D6E6;83951C;
        JSL.L PaletteTransition_StepTimeOfDayFade           ;82D6EA;80900C;
        JSL.L PaletteAnim_UpdatePointer42Slots                    ;82D6EE;808E69;
        JSL.L Input_DispatchByState                          ;82D6F2;84C034;
        JSL.L MapTilePatchRuntime_UpdateActiveSlots                          ;82D6F6;81A383;
        JSL.L PlayerAction_DispatchAndUpdateCamera                          ;82D6FA;81BFB7;
        JSL.L AutoMapScrolling               ;82D6FE;8095B3;
        JSL.L EventScript_UpdateAllActiveSlots                    ;82D702;84816F;
        JSL.L MapTilePatchScript_UpdateActive                    ;82D706;81A600;
        JSL.L GameOBJ_UpdateAnimationFrames                    ;82D70A;8582C7;
        JSL.L SpriteComponent_BuildVRAMUploadQueue                    ;82D70E;858CB2;
        JSL.L PrepareOAMData             ;82D712;8583E0;
        %Set16bit(!M)                             ;82D716;      ;
        ; PASS40_FIELD_STATUS_MENU_HANDOFF: this field loop accepts either
        ; the menu/status request bit or the forced-return bit in $7F1F60.
        LDA.L $7F1F5C                        ;82D718;7F1F5C;
        AND.W #$0004                         ;82D71C;      ;
        BNE .fieldStatusMenuOrReturnPending                      ;82D71F;82D731;
        LDA.L $7F1F60                        ;82D721;7F1F60;
        AND.W #$0080                         ;82D725;      ;
        BNE .fieldStatusMenuOrReturnPending                      ;82D728;82D731;
        %Set8bit(!M)                             ;82D72A;      ;
        STZ.B !NMI_Status                            ;82D72C;000000;
        JMP.W .Bank82_MainLogicBranch_82D6C9                    ;82D72E;82D6C9;

    .fieldStatusMenuOrReturnPending:
        %Set16bit(!MX)                             ;82D731;      ;
        LDX.W #$0000                         ;82D733;      ;

    .clearFieldMenuReturnScratch:
        %Set8bit(!M)                             ;82D736;      ;
        LDA.B #$00                           ;82D738;      ;
        STA.L !chicken_array,X                      ;82D73A;7EC286;
        INX                                  ;82D73E;      ;
        CPX.W #$0008                         ;82D73F;      ;
        BNE .clearFieldMenuReturnScratch                      ;82D742;82D736;
        %Set8bit(!M)                             ;82D744;      ;
        LDA.B !tilemap_to_load                            ;82D746;000022;
        CMP.B #$04                           ;82D748;      ;
        BCS .Bank82_MainLogicBranch_82D750                      ;82D74A;82D750;
        JSL.L CopyCurrentMaptoFarmMap                          ;82D74C;82A682;

    .Bank82_MainLogicBranch_82D750:
        %Set8bit(!M)                             ;82D750;      ;
        LDA.B #$09                           ;82D752;      ;
...
IntroScreen: ;82D75E
    ...
    .waitNMI:
        ...
        BEQ .natsumescreen
        CMP.B #$01
        BNE .Bank82_MainLogicBranch_82D829
        JML.L .parentsintroscene

    .Bank82_MainLogicBranch_82D829:
        CMP.B #$02                           ;82D829;      ;
        BNE .Bank82_MainLogicBranch_82D831                      ;82D82B;82D831;
        JML.L Bank82_MainLogicBranch_82DA8C                    ;82D82D;82DA8C;

    .Bank82_MainLogicBranch_82D831:
        CMP.B #$03                           ;82D831;      ;
        BNE .Bank82_MainLogicBranch_82D839                      ;82D833;82D839;
        JML.L Bank82_MainLogicBranch_82DAC9                    ;82D835;82DAC9;

    .Bank82_MainLogicBranch_82D839:
        CMP.B #$04                           ;82D839;      ;
        BNE .Bank82_MainLogicBranch_82D841                      ;82D83B;82D841;
        JML.L IntroScreen_MenuStage04PointerLayout                    ;82D83D;82DC0D;

    .Bank82_MainLogicBranch_82D841:
        CMP.B #$05                           ;82D841;      ;
        BNE .Bank82_MainLogicBranch_82D849                      ;82D843;82D849;
        JML.L IntroScreen_MenuStage05RuntimePointerRefresh                    ;82D845;82DD8C;

    .Bank82_MainLogicBranch_82D849:
        CMP.B #$06                           ;82D849;      ;
        BNE .Bank82_MainLogicBranch_82D851                      ;82D84B;82D851;
        JML.L IntroScreen_MenuStage06FadeOutToBootstrap                    ;82D84D;82DB8E;

    .Bank82_MainLogicBranch_82D851:
        CMP.B #$07                           ;82D851;      ;
        BNE .Bank82_MainLogicBranch_82D859                      ;82D853;82D859;
        JML.L Bank82_MainLogicBranch_82DF92                    ;82D855;82DF92;

    .Bank82_MainLogicBranch_82D859:
        CMP.B #$08                           ;82D859;      ;
        BNE .Bank82_MainLogicBranch_82D861                      ;82D85B;82D861;
        JML.L SaveLoadMenu_LoadSlotSelectSceneAndLoop                    ;82D85D;82E1F1;

    .Bank82_MainLogicBranch_82D861:
        CMP.B #$09                           ;82D861;      ;
        BNE .Bank82_MainLogicBranch_82D869                      ;82D863;82D869;
        JML.L IntroScreen_FieldStatusMenuStage09                    ;82D865;82DAF5;

    .Bank82_MainLogicBranch_82D869:
       CMP.B #$0A                           ;82D869;      ;
       BNE .democheck                      ;82D86B;82D871;
       JML.L NewGame_ResetSaveStateAndEnterFarmLoop                    ;82D86D;82D1C0;
    ...
    .parentsintroscene: ;82d8b2
        ...
        JSL.L MapLoad_LoadAreaByIdAndApplyOverlays                           ;82D936;80972C;
        %Set16bit(!MX)                             ;82D93A;      ;
        LDY.W #$0000                         ;82D93C;      ;

    .Bank82_MainLogicBranch_82D93F:
            %Set8bit(!M)                             ;82D93F;      ;
            LDA.B !NMI_Status                            ;82D941;000000;
            BEQ .Bank82_MainLogicBranch_82D93F                      ;82D943;82D93F;

        %Set16bit(!M)                             ;82D945;      ;
        LDA.W #$1800                         ;82D947;      ;
        STA.B $C7                            ;82D94A;0000C7;
        JSL.L MapTransition_BeginFadeAndLoadDestination         ;82D94C;809671;
        JSL.L MapTransition_CheckEdgeTriggerAndQueueDestination                           ;82D950;809A64;
        JSL.L Clock_UpdateAndDispatchHourlyEvents                     ;82D954;828000;
        JSL.L TextBox_UpdateRendererAndControlCodes                      ;82D958;83951C;
        JSL.L PaletteTransition_StepTimeOfDayFade           ;82D95C;80900C;
        JSL.L PaletteAnim_UpdatePointer42Slots                    ;82D960;808E69;
        JSL.L Input_DispatchByState                          ;82D964;84C034;
        JSL.L MapTilePatchRuntime_UpdateActiveSlots                          ;82D968;81A383;
        JSL.L PlayerAction_DispatchAndUpdateCamera                          ;82D96C;81BFB7;
        JSL.L AutoMapScrolling               ;82D970;8095B3;
        JSL.L EventScript_UpdateAllActiveSlots                    ;82D974;84816F;
        JSL.L MapTilePatchScript_UpdateActive                    ;82D978;81A600;
        JSL.L GameOBJ_UpdateAnimationFrames                    ;82D97C;8582C7;
        JSL.L SpriteComponent_BuildVRAMUploadQueue                    ;82D980;858CB2;
        JSL.L PrepareOAMData             ;82D984;8583E0;
        %Set16bit(!M)                             ;82D988;      ;
        LDA.L $7F1F5C                        ;82D98A;7F1F5C;
        AND.W #$0004                         ;82D98E;      ;
        BNE IntroScreen_FieldMenuRequestReentry                      ;82D991;82D9B3;
        %Set8bit(!M)                             ;82D993;      ;
        STZ.B !NMI_Status                            ;82D995;000000;
        JMP.W .Bank82_MainLogicBranch_82D93F                    ;82D997;82D93F;

        PLY                                  ;82D99A;      ;
        INY                                  ;82D99B;      ;
        CPY.W #$0168                         ;82D99C;      ;
        BNE .Bank82_MainLogicBranch_82D93F                      ;82D99F;82D93F;
        %Set8bit(!M)                             ;82D9A1;      ;
        LDA.B #$0F                           ;82D9A3;      ;
        ...
        JSL.L VideoFade_Out                  ;82D9AF;80880A;

IntroScreen_FieldMenuRequestReentry:
          %Set8bit(!M)                             ;82D9B3;      ;
                       LDA.B $95                            ;82D9B5;000095;
                       CMP.B #$09                           ;82D9B7;      ;
                       BNE Bank82_MainLogicBranch_82D9BE                      ;82D9B9;82D9BE;
                       JMP.W IntroScreen_democheck                    ;82D9BB;82D871;

          Bank82_MainLogicBranch_82D9BE:
          %Set8bit(!M)                             ;82D9BE;      ;
                       LDA.B #$0F                           ;82D9C0;      ;
                       ...
                       JSL.L VideoFade_Out                  ;82D9CC;80880A;
                       JSL.L ForceBlank                     ;82D9D0;808E0F;
                       %Set8bit(!M)                             ;82D9D4;      ;
                       LDA.B #$03                           ;82D9D6;      ;
                       JSL.L ManageGraphicPresets             ;82D9D8;808C59;
                       JSL.L VRAM_ClearAll                      ;82D9DC;808846;
                       JSL.L Palette_CGRAM_ClearAll                     ;82D9E0;808980;
                       JSL.L PaletteAnim_ClearAllPointer42Slots           ;82D9E4;808FAB;
                       JSL.L ClearSpriteDataTables         ;82D9E8;858ED7;
                       JSL.L InitializeOBJs         ;82D9EC;85820F;
                       JSL.L MapTilePatchRuntime_ClearAllSlots                 ;82D9F0;81A4C7;
                       JSL.L EventScript_ClearAllSlots                 ;82D9F4;848000;
                       %Set8bit(!M)                             ;82D9F8;      ;
                       LDA.B #$5B                           ;82D9FA;      ;
                       STA.B !tilemap_to_load                            ;82D9FC;000022;
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82D9FE;80A7C6;
                       %Set16bit(!M)                             ;82DA02;      ;
                       LDA.W #$006D                         ;82DA04;      ;
                       JSL.L Palette_LoadBGHalfToWRAM                      ;82DA07;8091CF;
                       %Set8bit(!M)                             ;82DA0B;      ;
                       %Set16bit(!X)                             ;82DA0D;      ;
                       LDA.B #$00                           ;82DA0F;      ;
                       ...
                       JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82DA4C;8095DE;
                       JSL.L AudioBGM_FadeOutPreviousTrackIfChanged                    ;82DA50;83841F;
                       JSL.L AudioBGM_StartCurrentTrackAndQueueSamples                    ;82DA54;8383A4;
                       JSL.L AudioBGM_FadeInCurrentTrackIfChanged                    ;82DA58;838380;
                       %Set8bit(!M)                             ;82DA5C;      ;
                       LDA.W $0110                          ;82DA5E;000110;
                       STA.W $0117                          ;82DA61;000117;
                       JSL.L ResetForceBlank                ;82DA64;808E1E;
                       JSL.L NMI_WaitForNextFrame               ;82DA68;808645;
                       %Set8bit(!M)                             ;82DA6C;      ;
                       LDA.B #$01                           ;82DA6E;      ;
                       ...
                       JSL.L VideoFade_In                         ;82DA7A;8087CE;
                       %Set8bit(!M)                             ;82DA7E;      ;
                       LDA.B #$02                           ;82DA80;      ;
          ...
          Bank82_MainLogicBranch_82DA8C:
          %Set8bit(!M)                             ;82DA8C;      ;
                       LDA.B $94                            ;82DA8E;000094;
                       AND.B #$01                           ;82DA90;      ;
                       BNE Bank82_MainLogicBranch_82DAA6                      ;82DA92;82DAA6;
                       %Set16bit(!M)                             ;82DA94;      ;
                       LDA.W !BG2_Map_Offset_X                          ;82DA96;000140;
                       INC A                                ;82DA99;      ;
                       STA.W !BG2_Map_Offset_X                          ;82DA9A;000140;
                       LDA.W !BG1_Map_Offset_X                          ;82DA9D;00013C;
                       BEQ Bank82_MainLogicBranch_82DAAE                      ;82DAA0;82DAAE;
                       DEC A                                ;82DAA2;      ;
                       STA.W !BG1_Map_Offset_X                          ;82DAA3;00013C;


          Bank82_MainLogicBranch_82DAA6:
          %Set8bit(!M)                             ;82DAA6;      ;
                       INC.B $94                            ;82DAA8;000094;
                       JML.L IntroScreen_democheck                    ;82DAAA;82D871;

          Bank82_MainLogicBranch_82DAAE:
          %Set8bit(!M)                             ;82DAAE;      ;
                       LDA.B $96                            ;82DAB0;000096;
                       INC A                                ;82DAB2;      ;
                       STA.B $96                            ;82DAB3;000096;
                       CMP.B #$3C                           ;82DAB5;      ;
                       BEQ Bank82_MainLogicBranch_82DABD                      ;82DAB7;82DABD;
                       JML.L Bank82_MainLogicBranch_82DAA6                    ;82DAB9;82DAA6;

          Bank82_MainLogicBranch_82DABD:
          %Set8bit(!M)                             ;82DABD;      ;
                       LDA.B #$03                           ;82DABF;      ;
          ...
          Bank82_MainLogicBranch_82DAC9:
          %Set8bit(!M)                             ;82DAC9;      ;
                       LDA.B $94                            ;82DACB;000094;
                       AND.B #$01                           ;82DACD;      ;
                       BNE Bank82_MainLogicBranch_82DAED                      ;82DACF;82DAED;
                       %Set16bit(!M)                             ;82DAD1;      ;
                       LDA.W !BG2_Map_Offset_X                          ;82DAD3;000140;
                       INC A                                ;82DAD6;      ;
                       STA.W !BG2_Map_Offset_X                          ;82DAD7;000140;
                       LDA.W !BG1_Map_Offset_Y                          ;82DADA;00013E;
                       BNE Bank82_MainLogicBranch_82DAE2                      ;82DADD;82DAE2;
                       JMP.W IntroScreen_MenuStage06FadeOutToBootstrap                    ;82DADF;82DB8E;

          Bank82_MainLogicBranch_82DAE2:
          DEC A                                ;82DAE2;      ;
          ...
          Bank82_MainLogicBranch_82DAED:
          %Set8bit(!M)                             ;82DAED;      ;
                       INC.B $94                            ;82DAEF;000094;
                       JML.L IntroScreen_democheck                    ;82DAF1;82D871;

; PASS40_FIELD_STATUS_STAGE09:
; Handler selected when !intro_stage/$95 == $09 after a field/status menu
; request. It reloads the shared menu graphics path and falls into the
; existing menu stage setup without claiming the whole visual layout yet.
IntroScreen_FieldStatusMenuStage09:
          %Set8bit(!M)                             ;82DAF5;      ;
                       LDA.B #$5C                           ;82DAF7;      ;
                       STA.B !tilemap_to_load                            ;82DAF9;000022;
                       JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82DAFB;8095DE;
                       JSL.L AudioBGM_FadeOutPreviousTrackFast                    ;82DAFF;838401;
                       %Set8bit(!M)                             ;82DB03;      ;
                       LDA.B #$0F                           ;82DB05;      ;
                       ...
                       JSL.L VideoFade_Out                  ;82DB11;80880A;
                       JSL.L ForceBlank                     ;82DB15;808E0F;
                       %Set8bit(!M)                             ;82DB19;      ;
                       LDA.B #$03                           ;82DB1B;      ;
                       JSL.L ManageGraphicPresets             ;82DB1D;808C59;
                       JSL.L VRAM_ClearAll                      ;82DB21;808846;
                       JSL.L Palette_CGRAM_ClearAll                     ;82DB25;808980;
                       JSL.L PaletteAnim_ClearAllPointer42Slots           ;82DB29;808FAB;
                       JSL.L ClearSpriteDataTables         ;82DB2D;858ED7;
                       JSL.L InitializeOBJs         ;82DB31;85820F;
                       JSL.L MapTilePatchRuntime_ClearAllSlots                 ;82DB35;81A4C7;
                       JSL.L EventScript_ClearAllSlots                 ;82DB39;848000;
                       %Set16bit(!M)                             ;82DB3D;      ;
                       ...
                       %Set8bit(!M)                             ;82DB4C;      ;
                       ; PASS41_FIELD_STATUS_MENU_VISUAL_BOOTSTRAP:
                       ; Stage $09 prepares shared menu/status visuals by
                       ; loading tilemap $5B, palette $006D and a $0200-byte
                       ; DMA block from $A9:D800 before switching to $5C.
                       LDA.B #$5B                           ;82DB4E;      ;
                       STA.B !tilemap_to_load                            ;82DB50;000022;
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82DB52;80A7C6;
                       %Set16bit(!M)                             ;82DB56;      ;
                       LDA.W #$006D                         ;82DB58;      ;
                       JSL.L Palette_LoadBGHalfToWRAM                ;82DB5B;8091CF;
                       %Set8bit(!M)                             ;82DB5F;      ;
                       %Set16bit(!X)                             ;82DB61;      ;
                       LDA.B #$00                           ;82DB63;      ;
                       ...
                       BRA IntroScreen_MenuVisualBootstrapAndFadeIn                      ;82DB8C;82DBB2;

IntroScreen_MenuStage06FadeOutToBootstrap:
          %Set8bit(!M)                             ;82DB8E;      ;
                       LDA.B #$5C                           ;82DB90;      ;
                       STA.B !tilemap_to_load                            ;82DB92;000022;
                       JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82DB94;8095DE;
                       JSL.L AudioBGM_FadeOutPreviousTrackFast                    ;82DB98;838401;
                       %Set8bit(!M)                             ;82DB9C;      ;
                       LDA.B #$0F                           ;82DB9E;      ;
                       ...
                       JSL.L VideoFade_Out                  ;82DBAA;80880A;
                       JSL.L ForceBlank                     ;82DBAE;808E0F;

; PASS41_SHARED_MENU_VISUAL_BOOTSTRAP:
; Common loader for menu/status visual stages. Uses the current
; !tilemap_to_load, resets BG/player transition state, reloads graphics/audio,
; fades in, then advances the shared dispatcher to stage $04.
IntroScreen_MenuVisualBootstrapAndFadeIn:
          %Set16bit(!M)                             ;82DBB2;      ;
                       ...
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82DBCF;80A7C6;
                       JSL.L AudioBGM_FadeOutPreviousTrackIfChanged                    ;82DBD3;83841F;
                       JSL.L AudioBGM_StartCurrentTrackAndQueueSamples                    ;82DBD7;8383A4;
                       JSL.L AudioBGM_FadeInCurrentTrackIfChanged                    ;82DBDB;838380;
                       %Set8bit(!M)                             ;82DBDF;      ;
                       LDA.W $0110                          ;82DBE1;000110;
                       STA.W $0117                          ;82DBE4;000117;
                       JSL.L ResetForceBlank                ;82DBE7;808E1E;
                       JSL.L NMI_WaitForNextFrame               ;82DBEB;808645;
                       %Set8bit(!M)                             ;82DBEF;      ;
                       LDA.B #$01                           ;82DBF1;      ;
                       ...
                       JSL.L VideoFade_In                         ;82DBFD;8087CE;
                       %Set8bit(!M)                             ;82DC01;      ;
                       LDA.B #$04                           ;82DC03;      ;
...
; PASS42_MENU_STAGE04_POINTER_LAYOUT:
; After the menu/status visual bootstrap, stage $04 waits $3C frames while
; nudging BG2, then clears/rebuilds Pointer42 slots 4-8. $098D selects one
; of three pointer layouts before the dispatcher advances to stage $05.
IntroScreen_MenuStage04PointerLayout:
          %Set8bit(!M)                             ;82DC0D;      ;
                       LDA.B $94                            ;82DC0F;000094;
                       AND.B #$01                           ;82DC11;      ;
                       BEQ IntroScreen_MenuStage04ScrollBg2                      ;82DC13;82DC19;
                       JML.L IntroScreen_MenuStage04WaitOrInstallLayout                    ;82DC15;82DC22;

IntroScreen_MenuStage04ScrollBg2:
          %Set16bit(!M)                             ;82DC19;      ;
                       LDA.W !BG2_Map_Offset_X                          ;82DC1B;000140;
                       INC A                                ;82DC1E;      ;
                       STA.W !BG2_Map_Offset_X                          ;82DC1F;000140;

IntroScreen_MenuStage04WaitOrInstallLayout:
          %Set8bit(!M)                             ;82DC22;      ;
                       LDA.B $94                            ;82DC24;000094;
                       CMP.B #$3C                           ;82DC26;      ;
                       BEQ IntroScreen_MenuStage04InstallPointerLayout                      ;82DC28;82DC32;
                       %Set8bit(!M)                             ;82DC2A;      ;
                       INC.B $94                            ;82DC2C;000094;
                       JML.L IntroScreen_democheck                    ;82DC2E;82D871;

IntroScreen_MenuStage04InstallPointerLayout:
          JSL.L PaletteAnim_ClearAllPointer42Slots           ;82DC32;808FAB;
                       %Set16bit(!M)                             ;82DC36;      ;
                       %Set8bit(!X)                             ;82DC38;      ;
                       LDA.W #$F2A4                         ;82DC3A;      ;
                       ...
                       LDX.B #$04                           ;82DC49;      ;
                       LDY.B #$07                           ;82DC4B;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DC4D;808E48;
                       %Set16bit(!M)                             ;82DC51;      ;
                       %Set8bit(!X)                             ;82DC53;      ;
                       LDA.W #$F2B5                         ;82DC55;      ;
                       ...
                       LDX.B #$05                           ;82DC64;      ;
                       LDY.B #$07                           ;82DC66;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DC68;808E48;
                       %Set8bit(!M)                             ;82DC6C;      ;
                       LDA.W $098D                          ;82DC6E;00098D;
                       CMP.B #$01                           ;82DC71;      ;
                       BNE IntroScreen_MenuStage04CheckVariant02                      ;82DC73;82DC79;
                       JML.L IntroScreen_MenuStage04LayoutVariant01                    ;82DC75;82DCD6;

IntroScreen_MenuStage04CheckVariant02:
          CMP.B #$02                           ;82DC79;      ;
                       BNE IntroScreen_MenuStage04LayoutVariant00                      ;82DC7B;82DC81;
                       JML.L IntroScreen_MenuStage04LayoutVariant02                    ;82DC7D;82DD2B;

IntroScreen_MenuStage04LayoutVariant00:
          %Set16bit(!M)                             ;82DC81;      ;
                       %Set8bit(!X)                             ;82DC83;      ;
                       LDA.W #$F2C9                         ;82DC85;      ;
                       ...
                       LDX.B #$06                           ;82DC94;      ;
                       LDY.B #$07                           ;82DC96;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DC98;808E48;
                       %Set16bit(!M)                             ;82DC9C;      ;
                       %Set8bit(!X)                             ;82DC9E;      ;
                       LDA.W #$F2DA                         ;82DCA0;      ;
                       ...
                       LDX.B #$07                           ;82DCAF;      ;
                       LDY.B #$07                           ;82DCB1;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DCB3;808E48;
                       %Set16bit(!M)                             ;82DCB7;      ;
                       %Set8bit(!X)                             ;82DCB9;      ;
                       LDA.W #$F2DA                         ;82DCBB;      ;
                       ...
                       LDX.B #$08                           ;82DCCA;      ;
                       LDY.B #$02                           ;82DCCC;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DCCE;808E48;
                       JML.L IntroScreen_MenuStage04AdvanceToStage05                    ;82DCD2;82DD80;

IntroScreen_MenuStage04LayoutVariant01:
          %Set16bit(!M)                             ;82DCD6;      ;
                       %Set8bit(!X)                             ;82DCD8;      ;
                       LDA.W #$F2DA                         ;82DCDA;      ;
                       ...
                       LDX.B #$06                           ;82DCE9;      ;
                       LDY.B #$07                           ;82DCEB;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DCED;808E48;
                       %Set16bit(!M)                             ;82DCF1;      ;
                       %Set8bit(!X)                             ;82DCF3;      ;
                       LDA.W #$F2C9                         ;82DCF5;      ;
                       ...
                       LDX.B #$07                           ;82DD04;      ;
                       LDY.B #$07                           ;82DD06;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DD08;808E48;
                       %Set16bit(!M)                             ;82DD0C;      ;
                       %Set8bit(!X)                             ;82DD0E;      ;
                       LDA.W #$F2DA                         ;82DD10;      ;
                       ...
                       LDX.B #$08                           ;82DD1F;      ;
                       LDY.B #$02                           ;82DD21;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DD23;808E48;
                       JML.L IntroScreen_MenuStage04AdvanceToStage05                    ;82DD27;82DD80;

IntroScreen_MenuStage04LayoutVariant02:
          %Set16bit(!M)                             ;82DD2B;      ;
                       %Set8bit(!X)                             ;82DD2D;      ;
                       LDA.W #$F2DA                         ;82DD2F;      ;
                       ...
                       LDX.B #$06                           ;82DD3E;      ;
                       LDY.B #$07                           ;82DD40;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DD42;808E48;
                       %Set16bit(!M)                             ;82DD46;      ;
                       %Set8bit(!X)                             ;82DD48;      ;
                       LDA.W #$F2DA                         ;82DD4A;      ;
                       ...
                       LDX.B #$07                           ;82DD59;      ;
                       LDY.B #$07                           ;82DD5B;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DD5D;808E48;
                       %Set16bit(!M)                             ;82DD61;      ;
                       %Set8bit(!X)                             ;82DD63;      ;
                       LDA.W #$F2C9                         ;82DD65;      ;
                       ...
                       LDX.B #$08                           ;82DD74;      ;
                       LDY.B #$02                           ;82DD76;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DD78;808E48;
                       JML.L IntroScreen_MenuStage04AdvanceToStage05                    ;82DD7C;82DD80;

IntroScreen_MenuStage04AdvanceToStage05:
          %Set8bit(!M)                             ;82DD80;      ;
                       LDA.B #$05                           ;82DD82;      ;
...
; PASS43_MENU_STAGE05_RUNTIME_POINTER_REFRESH:
; Stage $05 keeps the menu/status layout alive. It scrolls BG2, advances the
; frame counter, and only rebuilds Pointer42 slots 6-8 when $97 requests a
; visual refresh from the menu input handlers.
IntroScreen_MenuStage05RuntimePointerRefresh:
          %Set8bit(!M)                             ;82DD8C;      ;
                       LDA.B $94                            ;82DD8E;000094;
                       AND.B #$01                           ;82DD90;      ;
                       BNE IntroScreen_MenuStage05CheckRefreshFlag                      ;82DD92;82DD9D;
                       %Set16bit(!M)                             ;82DD94;      ;
                       LDA.W !BG2_Map_Offset_X                          ;82DD96;000140;
                       INC A                                ;82DD99;      ;
                       STA.W !BG2_Map_Offset_X                          ;82DD9A;000140;

IntroScreen_MenuStage05CheckRefreshFlag:
          %Set8bit(!M)                             ;82DD9D;      ;
                       INC.B $94                            ;82DD9F;000094;
                       LDA.B $97                            ;82DDA1;000097;
                       BNE IntroScreen_MenuStage05RefreshRequested                      ;82DDA3;82DDA9;
                       JML.L IntroScreen_MenuStage05AdvanceFrame                    ;82DDA5;82DEBD;

IntroScreen_MenuStage05RefreshRequested:
          STZ.B $97                            ;82DDA9;000097;
                       LDA.W $098D                          ;82DDAB;00098D;
                       CMP.B #$01                           ;82DDAE;      ;
                       BNE IntroScreen_MenuStage05CheckVariant02                      ;82DDB0;82DDB6;
                       JML.L IntroScreen_MenuStage05RefreshVariant01                    ;82DDB2;82DE13;

IntroScreen_MenuStage05CheckVariant02:
          CMP.B #$02                           ;82DDB6;      ;
                       BNE IntroScreen_MenuStage05RefreshVariant00                      ;82DDB8;82DDBE;
                       JML.L IntroScreen_MenuStage05RefreshVariant02                    ;82DDBA;82DE68;

IntroScreen_MenuStage05RefreshVariant00:
          %Set16bit(!M)                             ;82DDBE;      ;
                       %Set8bit(!X)                             ;82DDC0;      ;
                       LDA.W #$F2C9                         ;82DDC2;      ;
                       ...
                       LDX.B #$06                           ;82DDD1;      ;
                       LDY.B #$07                           ;82DDD3;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DDD5;808E48;
                       %Set16bit(!M)                             ;82DDD9;      ;
                       %Set8bit(!X)                             ;82DDDB;      ;
                       LDA.W #$F2DA                         ;82DDDD;      ;
                       ...
                       LDX.B #$07                           ;82DDEC;      ;
                       LDY.B #$07                           ;82DDEE;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DDF0;808E48;
                       %Set16bit(!M)                             ;82DDF4;      ;
                       %Set8bit(!X)                             ;82DDF6;      ;
                       LDA.W #$F2DA                         ;82DDF8;      ;
                       ...
                       LDX.B #$08                           ;82DE07;      ;
                       LDY.B #$02                           ;82DE09;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE0B;808E48;
                       JML.L IntroScreen_MenuStage05AdvanceFrame                    ;82DE0F;82DEBD;
                                                            ;      ;      ;
                                                            ;      ;      ;
IntroScreen_MenuStage05RefreshVariant01:
          %Set16bit(!M)                             ;82DE13;      ;
                       %Set8bit(!X)                             ;82DE15;      ;
                       LDA.W #$F2DA                         ;82DE17;      ;
                       ...
                       LDX.B #$06                           ;82DE26;      ;
                       LDY.B #$07                           ;82DE28;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE2A;808E48;
                       %Set16bit(!M)                             ;82DE2E;      ;
                       %Set8bit(!X)                             ;82DE30;      ;
                       LDA.W #$F2C9                         ;82DE32;      ;
                       ...
                       LDX.B #$07                           ;82DE41;      ;
                       LDY.B #$07                           ;82DE43;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE45;808E48;
                       %Set16bit(!M)                             ;82DE49;      ;
                       %Set8bit(!X)                             ;82DE4B;      ;
                       LDA.W #$F2DA                         ;82DE4D;      ;
                       ...
                       LDX.B #$08                           ;82DE5C;      ;
                       LDY.B #$02                           ;82DE5E;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE60;808E48;
                       JML.L IntroScreen_MenuStage05AdvanceFrame                    ;82DE64;82DEBD;
                                                            ;      ;      ;
                                                            ;      ;      ;
IntroScreen_MenuStage05RefreshVariant02:
          %Set16bit(!M)                             ;82DE68;      ;
                       %Set8bit(!X)                             ;82DE6A;      ;
                       LDA.W #$F2DA                         ;82DE6C;      ;
                       ...
                       LDX.B #$06                           ;82DE7B;      ;
                       LDY.B #$07                           ;82DE7D;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE7F;808E48;
                       %Set16bit(!M)                             ;82DE83;      ;
                       %Set8bit(!X)                             ;82DE85;      ;
                       LDA.W #$F2DA                         ;82DE87;      ;
                       ...
                       LDX.B #$07                           ;82DE96;      ;
                       LDY.B #$07                           ;82DE98;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DE9A;808E48;
                       %Set16bit(!M)                             ;82DE9E;      ;
                       %Set8bit(!X)                             ;82DEA0;      ;
                       LDA.W #$F2C9                         ;82DEA2;      ;
                       ...
                       LDX.B #$08                           ;82DEB1;      ;
                       LDY.B #$02                           ;82DEB3;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82DEB5;808E48;
                       JML.L IntroScreen_MenuStage05AdvanceFrame                    ;82DEB9;82DEBD;
                                                            ;      ;      ;
                                                            ;      ;      ;
IntroScreen_MenuStage05AdvanceFrame:
          %Set16bit(!MX)                             ;82DEBD;      ;
                       INC.B $90                            ;82DEBF;000090;
                       JML.L IntroScreen_democheck                    ;82DEC1;82D871; End of intro screen?
                                                            ;      ;      ;
                                                            ;      ;      ;
          ;;;;;;;; Pass 33: initializes the title/load scene used by continue/new game flow.
;;;;;;;; Loads title/load tilemaps, resets BG/player offsets, prepares palette/BGM,
;;;;;;;; and enters the load-slot menu state.
SaveLoadMenu_LoadSceneAndStaticBackground:
          %Set8bit(!M)                             ;82DEC5;      ;
                       LDA.B #$5C                           ;82DEC7;      ;
                       STA.B !tilemap_to_load                            ;82DEC9;000022;
                       JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82DECB;8095DE;
                       JSL.L AudioBGM_FadeOutPreviousTrackFast                    ;82DECF;838401;
                       %Set8bit(!M)                             ;82DED3;      ;
                       LDA.B #$0F                           ;82DED5;      ;
                       ...
                       JSL.L VideoFade_Out                  ;82DEE1;80880A;
                       JSL.L ForceBlank                     ;82DEE5;808E0F;
                       JSL.L VRAM_ClearAll                      ;82DEE9;808846;
                       %Set8bit(!M)                             ;82DEED;      ;
                       LDA.B #$03                           ;82DEEF;      ;
                       ...
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82DF16;80A7C6;
                       %Set8bit(!M)                             ;82DF1A;      ;
                       LDA.B #$5C                           ;82DF1C;      ;
                       STA.B !tilemap_to_load                            ;82DF1E;000022;
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82DF20;80A7C6;
                       %Set16bit(!M)                             ;82DF24;      ;
                       LDA.W #$006D                         ;82DF26;      ;
                       JSL.L Palette_LoadBGHalfToWRAM                ;82DF29;8091CF;
                       %Set8bit(!M)                             ;82DF2D;      ;
                       %Set16bit(!X)                             ;82DF2F;      ;
                       LDA.B #$00                           ;82DF31;      ;
                       ...
                       JSL.L AudioBGM_FadeOutPreviousTrackIfChanged                    ;82DF54;83841F;
                       JSL.L AudioBGM_StartCurrentTrackAndQueueSamples                    ;82DF58;8383A4;
                       JSL.L AudioBGM_FadeInCurrentTrackIfChanged                    ;82DF5C;838380;
                       %Set8bit(!M)                             ;82DF60;      ;
                       LDA.W $0110                          ;82DF62;000110;
                       STA.W $0117                          ;82DF65;000117;
                       JSL.L ResetForceBlank                ;82DF68;808E1E;
                       JSL.L NMI_WaitForNextFrame               ;82DF6C;808645;
                       %Set8bit(!M)                             ;82DF70;      ;
                       LDA.B #$01                           ;82DF72;      ;
                       ...
                       JSL.L VideoFade_In                         ;82DF7E;8087CE;
                       %Set8bit(!M)                             ;82DF82;      ;
                       LDA.B #$04                           ;82DF84;      ;
                       ...
                       JML.L IntroScreen_democheck                    ;82DF8E;82D871;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82DF92:
          %Set8bit(!M)                             ;82DF92;      ;
                       LDA.B #$5E                           ;82DF94;      ;
                       STA.B !tilemap_to_load                            ;82DF96;000022;
                       JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82DF98;8095DE;
                       JSL.L AudioBGM_FadeOutPreviousTrackFast                    ;82DF9C;838401;
                       %Set8bit(!M)                             ;82DFA0;      ;
                       LDA.B #$0F                           ;82DFA2;      ;
                       ...
                       JSL.L VideoFade_Out                  ;82DFAE;80880A;
                       JSL.L ForceBlank                     ;82DFB2;808E0F;
                       JSL.L VRAM_ClearAll                      ;82DFB6;808846;
                       %Set8bit(!M)                             ;82DFBA;      ;
                       LDA.B #$04                           ;82DFBC;      ;
                       JSL.L ManageGraphicPresets             ;82DFBE;808C59;
                       JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82DFC2;80A7C6;
                       %Set16bit(!M)                             ;82DFC6;      ;
                       LDA.W #$006F                         ;82DFC8;      ;
                       JSL.L Palette_LoadBGHalfToWRAM                ;82DFCB;8091CF;
                       %Set8bit(!M)                             ;82DFCF;      ;
                       %Set16bit(!X)                             ;82DFD1;      ;
                       LDA.B #$00                           ;82DFD3;      ;
                       ...
                       JSR.W SaveLoadMenu_DrawStaticTileBlocks                    ;82DFF6;82E742;
                       %Set16bit(!MX)                             ;82DFF9;      ;
                       LDA.W #$0000                         ;82DFFB;      ;
                       LDY.W #$0000                         ;82DFFE;      ;
                       JSR.W SaveLoadMenu_DrawSlotSummaryIfValid                    ;82E001;82E405;
                       %Set16bit(!MX)                             ;82E004;      ;
                       LDA.W #$0001                         ;82E006;      ;
                       LDY.W #$0000                         ;82E009;      ;
                       JSR.W SaveLoadMenu_DrawSlotSummaryIfValid                    ;82E00C;82E405;
                       JSL.L PaletteAnim_ClearAllPointer42Slots           ;82E00F;808FAB;
                       %Set16bit(!M)                             ;82E013;      ;
                       ...
                       JSL.L SaveSystem_LoadSlotSummary                      ;82E02F;83BA45;
                       %Set16bit(!MX)                             ;82E033;      ;
                       CPX.W #$0000                         ;82E035;      ;
                       BEQ Bank82_MainLogicBranch_82E03A                      ;82E038;82E03A;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E03A:
          %Set8bit(!M)                             ;82E03A;      ;
                       LDA.W $098E                          ;82E03C;00098E;
                       BEQ Bank82_MainLogicBranch_82E045                      ;82E03F;82E045;
                       JML.L Bank82_MainLogicBranch_82E07F                    ;82E041;82E07F;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E045:
          %Set16bit(!M)                             ;82E045;      ;
                       %Set8bit(!X)                             ;82E047;      ;
                       LDA.W #$F2E2                         ;82E049;      ;
                       ...
                       LDX.B #$04                           ;82E058;      ;
                       LDY.B #$00                           ;82E05A;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E05C;808E48;
                       %Set16bit(!M)                             ;82E060;      ;
                       %Set8bit(!X)                             ;82E062;      ;
                       LDA.W #$F2F9                         ;82E064;      ;
                       ...
                       LDX.B #$05                           ;82E073;      ;
                       LDY.B #$01                           ;82E075;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E077;808E48;
                       JML.L Bank82_MainLogicBranch_82E0B5                    ;82E07B;82E0B5;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E07F:
          %Set16bit(!M)                             ;82E07F;      ;
                       %Set8bit(!X)                             ;82E081;      ;
                       LDA.W #$F2F9                         ;82E083;      ;
                       ...
                       LDX.B #$04                           ;82E092;      ;
                       LDY.B #$00                           ;82E094;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E096;808E48;
                       %Set16bit(!M)                             ;82E09A;      ;
                       %Set8bit(!X)                             ;82E09C;      ;
                       LDA.W #$F2E2                         ;82E09E;      ;
                       ...
                       LDX.B #$05                           ;82E0AD;      ;
                       LDY.B #$01                           ;82E0AF;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E0B1;808E48;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E0B5:
          JSL.L AudioBGM_FadeOutPreviousTrackIfChanged                    ;82E0B5;83841F;
                       JSL.L AudioBGM_StartCurrentTrackAndQueueSamples                    ;82E0B9;8383A4;
                       JSL.L AudioBGM_FadeInCurrentTrackIfChanged                    ;82E0BD;838380;
                       %Set8bit(!M)                             ;82E0C1;      ;
                       LDA.W $0110                          ;82E0C3;000110;
                       STA.W $0117                          ;82E0C6;000117;
                       JSL.L ResetForceBlank                ;82E0C9;808E1E;
                       JSL.L NMI_WaitForNextFrame               ;82E0CD;808645;
                       %Set8bit(!M)                             ;82E0D1;      ;
                       LDA.B #$03                           ;82E0D3;      ;
                       ...
                       JSL.L VideoFade_In                         ;82E0DF;8087CE;
                       %Set8bit(!M)                             ;82E0E3;      ;
                       STZ.B $94                            ;82E0E5;000094;
                       STZ.B $96                            ;82E0E7;000096;
                       STZ.B $97                            ;82E0E9;000097;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E0EB:
          %Set8bit(!M)                             ;82E0EB;      ;
                       LDA.B !NMI_Status                            ;82E0ED;000000;
                       BEQ Bank82_MainLogicBranch_82E0EB                      ;82E0EF;82E0EB;
                       %Set16bit(!M)                             ;82E0F1;      ;
                       LDA.W #$1800                         ;82E0F3;      ;
                       STA.B $C7                            ;82E0F6;0000C7;
                       JSL.L PaletteAnim_UpdatePointer42Slots                    ;82E0F8;808E69;
                       JSL.L Input_DispatchByState                          ;82E0FC;84C034;
                       %Set8bit(!M)                             ;82E100;      ;
                       LDA.B $97                            ;82E102;000097;
                       BNE Bank82_MainLogicBranch_82E10A                      ;82E104;82E10A;
                       JML.L Bank82_MainLogicBranch_82E1BD                    ;82E106;82E1BD;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E10A:
          STZ.B $97                            ;82E10A;000097;
                       LDA.W $098E                          ;82E10C;00098E;
                       BEQ Bank82_MainLogicBranch_82E115                      ;82E10F;82E115;
                       JML.L Bank82_MainLogicBranch_82E16B                    ;82E111;82E16B;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E115:
          %Set16bit(!M)                             ;82E115;      ;
                       LDA.W #$0000                         ;82E117;      ;
                       JSL.L SaveSystem_LoadSlotSummary                      ;82E11A;83BA45;
                       %Set16bit(!MX)                             ;82E11E;      ;
                       CPX.W #$0000                         ;82E120;      ;
                       BEQ Bank82_MainLogicBranch_82E127                      ;82E123;82E127;
                       BRA Bank82_MainLogicBranch_82E131                      ;82E125;82E131;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E127:
          %Set16bit(!MX)                             ;82E127;      ;
                       LDA.W #$0000                         ;82E129;      ;
                       STA.W !BG1_Map_Offset_X                          ;82E12C;00013C;
                       BRA Bank82_MainLogicBranch_82E131                      ;82E12F;82E131;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E131:
          %Set16bit(!M)                             ;82E131;      ;
                       %Set8bit(!X)                             ;82E133;      ;
                       LDA.W #$F2E2                         ;82E135;      ;
                       ...
                       LDX.B #$04                           ;82E144;      ;
                       LDY.B #$00                           ;82E146;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E148;808E48;
                       %Set16bit(!M)                             ;82E14C;      ;
                       %Set8bit(!X)                             ;82E14E;      ;
                       LDA.W #$F2F9                         ;82E150;      ;
                       ...
                       LDX.B #$05                           ;82E15F;      ;
                       LDY.B #$01                           ;82E161;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E163;808E48;
                       JML.L Bank82_MainLogicBranch_82E1BD                    ;82E167;82E1BD;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E16B: %Set16bit(!M)                             ;82E16B;      ;
                       LDA.W #$0001                         ;82E16D;      ;
                       JSL.L SaveSystem_LoadSlotSummary                      ;82E170;83BA45;
                       %Set16bit(!MX)                             ;82E174;      ;
                       CPX.W #$0000                         ;82E176;      ;
                       BEQ Bank82_MainLogicBranch_82E17D                      ;82E179;82E17D;
                       BRA Bank82_MainLogicBranch_82E187                      ;82E17B;82E187;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E17D: %Set16bit(!MX)                             ;82E17D;      ;
                       LDA.W #$0000                         ;82E17F;      ;
                       STA.W !BG1_Map_Offset_X                          ;82E182;00013C;
                       BRA Bank82_MainLogicBranch_82E187                      ;82E185;82E187;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E187: %Set16bit(!M)                             ;82E187;      ;
                       %Set8bit(!X)                             ;82E189;      ;
                       LDA.W #$F2F9                         ;82E18B;      ;
                       ...
                       LDX.B #$04                           ;82E19A;      ;
                       LDY.B #$00                           ;82E19C;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E19E;808E48;
                       %Set16bit(!M)                             ;82E1A2;      ;
                       %Set8bit(!X)                             ;82E1A4;      ;
                       LDA.W #$F2E2                         ;82E1A6;      ;
                       ...
                       LDX.B #$05                           ;82E1B5;      ;
                       LDY.B #$01                           ;82E1B7;      ;
                       JSL.L PaletteAnim_SetPointer42Slot            ;82E1B9;808E48;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E1BD: %Set8bit(!M)                             ;82E1BD;      ;
                       LDA.B $96                            ;82E1BF;000096;
                       CMP.B #$01                           ;82E1C1;      ;
                       BNE Bank82_MainLogicBranch_82E1D1                      ;82E1C3;82E1D1;
                       STZ.B $96                            ;82E1C5;000096;
                       %Set16bit(!M)                             ;82E1C7;      ;
                       INC.W !BG2_Map_Offset_X                          ;82E1C9;000140;
                       DEC.W !BG2_Map_Offset_Y                          ;82E1CC;000142;
                       BRA Bank82_MainLogicBranch_82E1D5                      ;82E1CF;82E1D5;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E1D1: %Set8bit(!M)                             ;82E1D1;      ;
                       INC.B $96                            ;82E1D3;000096;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E1D5: %Set8bit(!M)                             ;82E1D5;      ;
                       LDA.B $94                            ;82E1D7;000094;
                       CMP.B #$01                           ;82E1D9;      ;
                       BNE Bank82_MainLogicBranch_82E1E1                      ;82E1DB;82E1E1;
                       JML.L SaveLoadMenu_LoadSceneAndStaticBackground                    ;82E1DD;82DEC5;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E1E1: CMP.B #$02                           ;82E1E1;      ;
                       BNE Bank82_MainLogicBranch_82E1E9                      ;82E1E3;82E1E9;
                       JML.L StartFromNewGame                    ;82E1E5;82E7F9;
                                                            ;      ;      ;
                                                            ;      ;      ;
          Bank82_MainLogicBranch_82E1E9: %Set8bit(!M)                             ;82E1E9;      ;
                       STZ.B !NMI_Status                            ;82E1EB;000000;
                       JML.L Bank82_MainLogicBranch_82E0EB                    ;82E1ED;82E0EB;
;;;;;;;;
SaveLoadMenu_LoadSlotSelectSceneAndLoop:
        %Set8bit(!M)                             ;82E1F1;      ;
        LDA.B #$5E                           ;82E1F3;      ;
        STA.B !tilemap_to_load                            ;82E1F5;000022;
        JSL.L AudioBGM_SelectTrackForAreaSeasonTime;82E1F7;8095DE;
        JSL.L AudioBGM_FadeOutPreviousTrackFast                    ;82E1FB;838401;
        %Set8bit(!M)                             ;82E1FF;      ;
        LDA.B #$0F                           ;82E201;      ;
        ...
        JSL.L VideoFade_Out                  ;82E20D;80880A;
        JSL.L ForceBlank                     ;82E211;808E0F;
        JSL.L VRAM_ClearAll                      ;82E215;808846;
        %Set8bit(!M)                             ;82E219;      ;
        LDA.B #$04                           ;82E21B;      ;
        JSL.L ManageGraphicPresets             ;82E21D;808C59;
        JSL.L MapLoad_DecompressAreaGraphicsAndSetCamera           ;82E221;80A7C6;
        %Set16bit(!M)                             ;82E225;      ;
        LDA.W #$006F                         ;82E227;      ;
        JSL.L Palette_LoadBGHalfToWRAM                ;82E22A;8091CF;
        %Set8bit(!M)                             ;82E22E;      ;
        %Set16bit(!X)                             ;82E230;      ;
        LDA.B #$00                           ;82E232;      ;
        ...
        JSR.W SaveLoadMenu_DrawStaticTileBlocks                    ;82E255;82E742;
        %Set16bit(!MX)                             ;82E258;      ;
        LDA.W #$0000                         ;82E25A;      ;
        LDY.W #$0001                         ;82E25D;      ;
        JSR.W SaveLoadMenu_DrawSlotSummaryIfValid                    ;82E260;82E405;
        %Set16bit(!MX)                             ;82E263;      ;
        LDA.W #$0001                         ;82E265;      ;
        LDY.W #$0001                         ;82E268;      ;
        JSR.W SaveLoadMenu_DrawSlotSummaryIfValid                    ;82E26B;82E405;
        JSL.L PaletteAnim_ClearAllPointer42Slots           ;82E26E;808FAB;
        %Set8bit(!M)                             ;82E272;      ;
        LDA.W $098E                          ;82E274;00098E;
        BEQ Bank82_MainLogicBranch_82E27D                      ;82E277;82E27D;
        JML.L Bank82_MainLogicBranch_82E2B7                    ;82E279;82E2B7;

    Bank82_MainLogicBranch_82E27D:
        %Set16bit(!M)                             ;82E27D;      ;
        %Set8bit(!X)                             ;82E27F;      ;
        LDA.W #$F2E2                         ;82E281;      ;
        ...
        LDX.B #$04                           ;82E290;      ;
        LDY.B #$00                           ;82E292;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E294;808E48;
        %Set16bit(!M)                             ;82E298;      ;
        %Set8bit(!X)                             ;82E29A;      ;
        LDA.W #$F2F9                         ;82E29C;      ;
        ...
        LDX.B #$05                           ;82E2AB;      ;
        LDY.B #$01                           ;82E2AD;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E2AF;808E48;
        JML.L Bank82_MainLogicBranch_82E2ED                    ;82E2B3;82E2ED;

    Bank82_MainLogicBranch_82E2B7:
        %Set16bit(!M)                             ;82E2B7;      ;
        %Set8bit(!X)                             ;82E2B9;      ;
        LDA.W #$F2F9                         ;82E2BB;      ;
        ...
        LDX.B #$04                           ;82E2CA;      ;
        LDY.B #$00                           ;82E2CC;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E2CE;808E48;
        %Set16bit(!M)                             ;82E2D2;      ;
        %Set8bit(!X)                             ;82E2D4;      ;
        LDA.W #$F2E2                         ;82E2D6;      ;
        ...
        LDX.B #$05                           ;82E2E5;      ;
        LDY.B #$01                           ;82E2E7;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E2E9;808E48;

    Bank82_MainLogicBranch_82E2ED:
        %Set16bit(!M)                             ;82E2ED;      ;
        ...
        JSL.L AudioBGM_FadeOutPreviousTrackIfChanged                    ;82E301;83841F;
        JSL.L AudioBGM_StartCurrentTrackAndQueueSamples                    ;82E305;8383A4;
        JSL.L AudioBGM_FadeInCurrentTrackIfChanged                    ;82E309;838380;
        %Set8bit(!M)                             ;82E30D;      ;
        LDA.W $0110                          ;82E30F;000110;
        STA.W $0117                          ;82E312;000117;
        JSL.L ResetForceBlank                ;82E315;808E1E;
        JSL.L NMI_WaitForNextFrame               ;82E319;808645;
        %Set8bit(!M)                             ;82E31D;      ;
        LDA.B #$03                           ;82E31F;      ;
        ...
        JSL.L VideoFade_In                         ;82E32B;8087CE;
        %Set8bit(!M)                             ;82E32F;      ;
        STZ.B $94                            ;82E331;000094;
        STZ.B $96                            ;82E333;000096;
        STZ.B $97                            ;82E335;000097;

    Bank82_MainLogicBranch_82E337:
        %Set8bit(!M)                             ;82E337;      ;
        LDA.B !NMI_Status                            ;82E339;000000;
        BEQ Bank82_MainLogicBranch_82E337                      ;82E33B;82E337;

        %Set16bit(!M)                             ;82E33D;      ;
        LDA.W #$1800                         ;82E33F;      ;
        STA.B $C7                            ;82E342;0000C7;
        JSL.L PaletteAnim_UpdatePointer42Slots                    ;82E344;808E69;
        JSL.L Input_DispatchByState                          ;82E348;84C034;
        %Set8bit(!M)                             ;82E34C;      ;
        LDA.B $97                            ;82E34E;000097;
        BNE Bank82_MainLogicBranch_82E356                      ;82E350;82E356;
        JML.L SaveLoadMenu_SlotSelectLoopTick                    ;82E352;82E3D1;

    Bank82_MainLogicBranch_82E356:
        STZ.B $97                            ;82E356;000097;
        LDA.W $098E                          ;82E358;00098E;
        BEQ Bank82_MainLogicBranch_82E361                      ;82E35B;82E361;
        JML.L Bank82_MainLogicBranch_82E39B                    ;82E35D;82E39B;

    Bank82_MainLogicBranch_82E361:
        %Set16bit(!M)                             ;82E361;      ;
        %Set8bit(!X)                             ;82E363;      ;
        LDA.W #$F2E2                         ;82E365;      ;
        ...
        LDX.B #$04                           ;82E374;      ;
        LDY.B #$00                           ;82E376;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E378;808E48;
        %Set16bit(!M)                             ;82E37C;      ;
        %Set8bit(!X)                             ;82E37E;      ;
        LDA.W #$F2F9                         ;82E380;      ;
        ...
        LDX.B #$05                           ;82E38F;      ;
        LDY.B #$01                           ;82E391;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E393;808E48;
        JML.L SaveLoadMenu_SlotSelectLoopTick                    ;82E397;82E3D1;

    Bank82_MainLogicBranch_82E39B:
        %Set16bit(!M)                             ;82E39B;      ;
        %Set8bit(!X)                             ;82E39D;      ;
        LDA.W #$F2F9                         ;82E39F;      ;
        ...
        LDX.B #$04                           ;82E3AE;      ;
        LDY.B #$00                           ;82E3B0;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E3B2;808E48;
        %Set16bit(!M)                             ;82E3B6;      ;
        %Set8bit(!X)                             ;82E3B8;      ;
        LDA.W #$F2E2                         ;82E3BA;      ;
        ...
        LDX.B #$05                           ;82E3C9;      ;
        LDY.B #$01                           ;82E3CB;      ;
        JSL.L PaletteAnim_SetPointer42Slot            ;82E3CD;808E48;

;;;;;;;;
SaveLoadMenu_SlotSelectLoopTick:
        %Set8bit(!M)                             ;82E3D1;      ;
        LDA.B $96                            ;82E3D3;000096;
        CMP.B #$01                           ;82E3D5;      ;
        BNE Bank82_MainLogicBranch_82E3E5                      ;82E3D7;82E3E5;
        STZ.B $96                            ;82E3D9;000096;
        %Set16bit(!M)                             ;82E3DB;      ;
        INC.W !BG2_Map_Offset_X                          ;82E3DD;000140;
        DEC.W !BG2_Map_Offset_Y                          ;82E3E0;000142;
        BRA Bank82_MainLogicBranch_82E3E9                      ;82E3E3;82E3E9;

    Bank82_MainLogicBranch_82E3E5:
        %Set8bit(!M)                             ;82E3E5;      ;
        INC.B $96                            ;82E3E7;000096;

    Bank82_MainLogicBranch_82E3E9:
        %Set8bit(!M)                             ;82E3E9;      ;
        LDA.B $94                            ;82E3EB;000094;
        CMP.B #$01                           ;82E3ED;      ;
        BNE Bank82_MainLogicBranch_82E3F5                      ;82E3EF;82E3F5;
        JML.L SaveLoadMenu_LoadSceneAndStaticBackground                    ;82E3F1;82DEC5;

    Bank82_MainLogicBranch_82E3F5:
        CMP.B #$02                           ;82E3F5;      ;
        BNE Bank82_MainLogicBranch_82E3FD                      ;82E3F7;82E3FD;
        JML.L StartFromLoad                    ;82E3F9;82E7E9;

    Bank82_MainLogicBranch_82E3FD:
        %Set8bit(!M)                             ;82E3FD;      ;
        STZ.B !NMI_Status                            ;82E3FF;000000;
        JML.L Bank82_MainLogicBranch_82E337                    ;82E401;82E337;

    ;;;;;;;; Pass 33: draws the summary for one valid save slot.
;;;;;;;; It loads the slot summary through SaveSystem_LoadSlotSummary and renders
;;;;;;;; player name, year, day and weekday/season name glyphs into fixed VRAM slots.
SaveLoadMenu_DrawSlotSummaryIfValid:
        %Set16bit(!MX)                             ;82E405;      ;
        PHA                                  ;82E407;      ;
        PHY                                  ;82E408;      ;
        JSL.L SaveSystem_LoadSlotSummary                      ;82E409;83BA45;
        %Set16bit(!MX)                             ;82E40D;      ;
        PLY                                  ;82E40F;      ;
        PLA                                  ;82E410;      ;
        CPX.W #$0000                         ;82E411;      ;
        BNE Bank82_MainLogicBranch_82E41A                      ;82E414;82E41A;
        JML.L SaveLoadMenu_ClearSlotSummaryArea                    ;82E416;82E5E7;

    Bank82_MainLogicBranch_82E41A:
        ASL A                                ;82E41A;      ;
        ASL A                                ;82E41B;      ;
        ASL A                                ;82E41C;      ;
        TAX                                  ;82E41D;      ;
        LDA.L SaveLoadMenu_SlotSummaryVRAMPositionTable,X                ;82E41E;82F278;
        PHX                                  ;82E422;      ;
        STA.B $82                            ;82E423;000082;
        LDX.W #$0000                         ;82E425;      ;

    Bank82_MainLogicBranch_82E428:
        %Set16bit(!M)                             ;82E428;      ;
        PHX                                  ;82E42A;      ;
        ...
        LDX.W #$0000                         ;82E439;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E43C;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E440;808AF0;
        %Set16bit(!MX)                             ;82E444;      ;
        ...
        INX                                  ;82E44F;      ;
        CPX.W #$0004                         ;82E450;      ;
        BNE Bank82_MainLogicBranch_82E428                      ;82E453;82E428;

        %Set16bit(!MX)                             ;82E455;      ;
        PLX                                  ;82E457;      ;
        INX                                  ;82E458;      ;
        INX                                  ;82E459;      ;
        LDA.L SaveLoadMenu_SlotSummaryVRAMPositionTable,X                ;82E45A;82F278;
        PHX                                  ;82E45E;      ;
        ...
        JSL.L TextBox_RenderNumberToken                    ;82E481;8397A6;
        JSL.L DMA_StartProgrammedTransfer           ;82E485;808AF0;
        %Set16bit(!MX)                             ;82E489;      ;
        ...
        ASL A                                ;82E4A5;      ;
        TAX                                  ;82E4A6;      ;
        LDA.L SaveLoadMenu_YearOrdinalGlyphTable,X                ;82E4A7;82F298;
        LDX.W #$0000                         ;82E4AB;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E4AE;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E4B2;808AF0;
        %Set16bit(!MX)                             ;82E4B6;      ;
        ...
        ADC.W #$0002                         ;82E4D4;      ;
        TAX                                  ;82E4D7;      ;
        LDA.L SaveLoadMenu_YearOrdinalGlyphTable,X                ;82E4D8;82F298;
        LDX.W #$0000                         ;82E4DC;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E4DF;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E4E3;808AF0;
        %Set16bit(!MX)                             ;82E4E7;      ;
        PLX                                  ;82E4E9;      ;
        INX                                  ;82E4EA;      ;
        INX                                  ;82E4EB;      ;
        LDA.L SaveLoadMenu_SlotSummaryVRAMPositionTable,X                ;82E4EC;82F278;
        PHX                                  ;82E4F0;      ;
        ...
        JSL.L TextBox_RenderNumberToken                    ;82E512;8397A6;
        JSL.L DMA_StartProgrammedTransfer           ;82E516;808AF0;
        %Set16bit(!MX)                             ;82E51A;      ;
        PLX                                  ;82E51C;      ;
        LDA.L SaveLoadMenu_SlotSummaryVRAMPositionTable,X                ;82E51D;82F278;
        PHX                                  ;82E521;      ;
        CLC                                  ;82E522;      ;
        ADC.W #$0008                         ;82E523;      ;
        ...
        JSL.L TextBox_RenderNumberToken                    ;82E531;8397A6;
        JSL.L DMA_StartProgrammedTransfer           ;82E535;808AF0;
        %Set16bit(!MX)                             ;82E539;      ;
        JSL.L Calendar_LoadDateNameBuffers        ;82E53B;8289D6;
        %Set16bit(!M)                             ;82E53F;      ;
        PLX                                  ;82E541;      ;
        INX                                  ;82E542;      ;
        INX                                  ;82E543;      ;
        LDA.L SaveLoadMenu_SlotSummaryVRAMPositionTable,X                ;82E544;82F278;
        STA.L $800185                        ;82E548;800185;
        LDA.W $08B3                          ;82E54C;0008B3;
        LDX.W #$0000                         ;82E54F;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E552;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E556;808AF0;
        %Set16bit(!MX)                             ;82E55A;      ;
        ...
        LDX.W #$0000                         ;82E56B;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E56E;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E572;808AF0;
        %Set16bit(!MX)                             ;82E576;      ;
        ...
        LDX.W #$0000                         ;82E587;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E58A;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E58E;808AF0;
        %Set16bit(!MX)                             ;82E592;      ;
        ...
        LDX.W #$0000                         ;82E5A3;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E5A6;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E5AA;808AF0;
        %Set16bit(!MX)                             ;82E5AE;      ;
        ...
        LDX.W #$0000                         ;82E5BF;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E5C2;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E5C6;808AF0;
        %Set16bit(!MX)                             ;82E5CA;      ;
        ...
        LDX.W #$0000                         ;82E5DB;      ;
        JSL.L TextBox_QueueGlyphDMA                    ;82E5DE;839823;
        JSL.L DMA_StartProgrammedTransfer           ;82E5E2;808AF0;

        RTS                                  ;82E5E6;      ;

;;;;;;;;
;;;;;;;; Pass 33: clears the visual area for an empty/invalid save slot.
;;;;;;;; Uses a blank tile pattern table and DMA to overwrite the slot summary region.
SaveLoadMenu_ClearSlotSummaryArea:
        %Set16bit(!MX)
        ASL A
        TAX
        LDA.L SaveLoadMenu_ClearAreaBaseOffsetTable,X
        STA.B $80
        STZ.B $82
        STY.B $84
    ...
    .outerloop:
        ...
        STZ.B $7E
        LDA.B $84
        BEQ .Bank82_MainLogicBranch_82E608
        LDA.W #$006C
        STA.B $7E

    .Bank82_MainLogicBranch_82E608:
        %Set16bit(!MX)
        LDA.W #$0000
...
SaveLoadMenu_BlankTilePatternTable:   dw $02FF,$02FF,$002C,$006E,$00C2,$02FF,$0008,$0086;82E666;      ;
...
SaveLoadMenu_ClearAreaBaseOffsetTable:   db $E8,$10,$28,$12                   ;82E73E;      ;

;;;;;;;; Pass 33: draws static tile blocks for the load/menu screen frame.
;;;;;;;; The source/destination/count tables below describe small repeated DMA blocks.
SaveLoadMenu_DrawStaticTileBlocks:
        %Set16bit(!MX)
        LDX.W #$0000

    .outerloop:
        %Set16bit(!M)
        LDA.L SaveLoadMenu_StaticTileVRAMDestTable,X
        STA.B $7E
        INC A
        STA.B $80
        CLC
        ...
        INC A
        STA.B $84
        LDA.L SaveLoadMenu_StaticTileSourceOffsetTable,X
        STA.B $86
        LDY.W #$0000
        ...
        .innerloop:
            ...
            PLY
            INY
            LDA.L SaveLoadMenu_StaticTileBlockCountTable,X
            STA.B $88
            CPY.B $88
            BNE .innerloop
...
;;;;;;;;
NameEntryScene_UnusedZero099FAndLoad:
        %Set8bit(!M)
        LDA.B #$00
        STA.W $099F
        JML.L NameEntryScene_LoadAndRun

; PASS47_NAME_ENTRY_CORE: full naming scene loader/loop.
; tilemap $5F, inputstate $05, three character pages, 4-byte temp name buffer,
; animated cursor GameOBJ and final dispatch to SetPlayerName/SetCowName/etc.
NameEntryScene_LoadAndRun: ;82E80C
        %Set8bit(!M)
        LDA.B #$5F
        STA.B !tilemap_to_load
        JSL.L AudioBGM_SelectTrackForAreaSeasonTime
        ...
        LDA.B $A5
        STA.W $0995
        JSL.L NameEntryUI_DrawNameSlotFrameDMA
        JSL.L NameEntryUI_ClearAllNameSlotGlyphs
        %Set16bit(!M)
        STZ.W !BG1_Map_Offset_X
        STZ.W !BG1_Map_Offset_Y
        STZ.W !BG2_Map_Offset_X
    ...
    .waitNMI:
        ...
        LDA.W #$1800
        STA.B $C7
        JSR.W .Bank82_MainLogicBranch_82EA15
        JSL.L Input_DispatchByState
        %Set8bit(!M)
        LDA.W $0993
        CMP.B #$03
        BNE .Bank82_MainLogicBranch_82E97F
        JMP.W .Bank82_MainLogicBranch_82E9CE

    .Bank82_MainLogicBranch_82E97F:
        JSL.L NameEntryCursor_UpdateSpritePositionFromGrid
        %Set8bit(!M)
        LDA.B $96
        CMP.B #$01
        BNE .Bank82_MainLogicBranch_82E997
        STZ.B $96
        %Set16bit(!M)
        INC.W !BG2_Map_Offset_X
        DEC.W !BG2_Map_Offset_Y
        BRA .Bank82_MainLogicBranch_82E99B

    .Bank82_MainLogicBranch_82E997:
        %Set8bit(!M)
        INC.B $96

    .Bank82_MainLogicBranch_82E99B:
        %Set16bit(!M)
        LDA.W $0997
        STA.B $A1
        LDA.W $0999
    ...
    .Bank82_MainLogicBranch_82E9CE:
        ...
        LDA.W $099F
        CMP.B #$00
        BNE .Bank82_MainLogicBranch_82E9E3
        JML.L SetPlayerName

    .Bank82_MainLogicBranch_82E9E3:
        CMP.B #$01
        BNE .Bank82_MainLogicBranch_82E9EB
        JML.L SetCowNameBought

    .Bank82_MainLogicBranch_82E9EB:
        CMP.B #$02
        BNE .Bank82_MainLogicBranch_82E9F3
        JML.L SetCowNameBorn

    .Bank82_MainLogicBranch_82E9F3:
        CMP.B #$03
        BNE .Bank82_MainLogicBranch_82E9FB
        JML.L SetDogName

    .Bank82_MainLogicBranch_82E9FB:
        CMP.B #$04
        BNE .Bank82_MainLogicBranch_82EA03
        JML.L SetHorseName

    .Bank82_MainLogicBranch_82EA03:
        CMP.B #$05
        BNE .Bank82_MainLogicBranch_82EA0B
        JML.L SetKid1Name

    .Bank82_MainLogicBranch_82EA0B:
        CMP.B #$06
        BNE .Bank82_MainLogicBranch_82EA13
        JML.L SetKid2Name

    .Bank82_MainLogicBranch_82EA13:
        BRA .Bank82_MainLogicBranch_82E9CE

    .Bank82_MainLogicBranch_82EA15:
        %Set8bit(!M)
        %Set16bit(!X)
        ...
        AND.W #$007F
        CMP.W #$0014
        BNE .Bank82_MainLogicBranch_82EA39
        %Set8bit(!M)
        LDA.W $018B
        AND.B #$80
        EOR.B #$80
    ...
    .Bank82_MainLogicBranch_82EA39:
        %Set8bit(!M)
        LDA.W $018B
        AND.B #$80
        BNE .Bank82_MainLogicBranch_82EA4D
        %Set16bit(!M)
        LDA.W #$00A8
        JSL.L NameEntryUI_DrawGlyphIntoCurrentSlot
        BRA .Bank82_MainLogicBranch_82EA56

    .Bank82_MainLogicBranch_82EA4D:
        %Set16bit(!M)
        LDA.W #$00B1
        JSL.L NameEntryUI_DrawGlyphIntoCurrentSlot

    .Bank82_MainLogicBranch_82EA56:
        %Set8bit(!M)
        LDA.W $018B
        INC A
        STA.W $018B
...
;;;;;;;;
; PASS47_NAME_ENTRY_CORE: queue one glyph tile into the active name slot.
; A = glyph tile id. Slot VRAM target comes from NameEntryUI_NameSlotGlyphVRAMPositionTable.
NameEntryUI_DrawGlyphIntoCurrentSlot:
        ...
        ASL A
        TAX
        LDA.L NameEntryUI_NameSlotGlyphVRAMPositionTable,X
        STA.L $800185
        PLA
        LDX.W #$0001
        JSL.L TextBox_QueueGlyphDMA

        RTL

;;;;;;;;
; PASS47_NAME_ENTRY_CORE: clear the four visible name slots with blank glyph $00A8.
NameEntryUI_ClearAllNameSlotGlyphs:
        %Set16bit(!M)
        LDA.L NameEntryUI_NameSlotGlyphVRAMPositionTable
        STA.B $82
        LDX.W #$0000
...
;;;;;;;;
; PASS47_NAME_ENTRY_CORE: DMA the static frame/underline pieces for the four name slots.
NameEntryUI_DrawNameSlotFrameDMA:
        %Set16bit(!MX)
        LDA.L NameEntryUI_NameSlotFrameVRAMBase
        STA.B $7E
        INC A
        STA.B $80
        CLC
        ...
        INC A
        STA.B $84
        LDA.L NameEntryUI_NameSlotFrameSourceOffset
        STA.B $86
        LDY.W #$0000

    .Bank82_MainLogicBranch_82EAD1:
        PHY
        %Set8bit(!M)
        LDA.B #$00
        STA.B !ProgDMA_Channel_Index
        ...
        PLY
        INY
        LDA.L NameEntryUI_NameSlotCount
        STA.B $88
        CPY.B $88
        BNE .Bank82_MainLogicBranch_82EAD1

        RTS

; PASS47_NAME_ENTRY_CORE: static name-slot frame DMA parameters.
NameEntryUI_NameSlotFrameSourceOffset: dw $788C                             ;82EB49;      ;
NameEntryUI_NameSlotFrameVRAMBase: dw $2002                             ;82EB4B;      ;
NameEntryUI_NameSlotGlyphVRAMPositionTable: dw $5010,$5020,$5030,$5040           ;82EB4D;      ;
NameEntryUI_NameSlotCount: dw $0004                             ;82EB55;      ;

;;;;;;;;
; PASS47_NAME_ENTRY_CORE: update cursor GameOBJ position from the current page/table entry.
; $0993 selects page: 0 uppercase, 1 lowercase, 2 symbols/control.
NameEntryCursor_UpdateSpritePositionFromGrid:
        ...
        STA.B $74
        LDA.W $0993
        BEQ .Bank82_MainLogicBranch_82EB89
        CMP.B #$01
        BEQ .Bank82_MainLogicBranch_82EB7C
        %Set16bit(!M)
        LDA.W #$F0D0
        STA.B $72
        %Set8bit(!M)
        LDA.B #$82
        STA.B $74
        BRA .Bank82_MainLogicBranch_82EB89

    .Bank82_MainLogicBranch_82EB7C:
        %Set16bit(!M)
        LDA.W #$EE30
        STA.B $72
        %Set8bit(!M)
...
; PASS47_NAME_ENTRY_CORE: read one field from the current 8-byte grid entry.
; Input A selects field: 0 down, 1 up, 2 left, 3 right, 4 glyph, 5 action/control.
NameEntryGrid_ReadCurrentEntryField: ;82EBAC
        %Set16bit(!MX)
        PHA
        LDA.W #$EBF8
        STA.B $72
...
; PASS47_NAME_ENTRY_CORE: name-entry keyboard grid. Each 8-byte entry is:
; X, Y, DownIndex, UpIndex, RightIndex, LeftIndex, GlyphOrTile, Action.
; The code uses page base $82:EBF8 for uppercase, $82:EE30 for lowercase, $82:F0D0 for symbols/control.
NameEntryGrid_PageDataTable:  db $28,$44,$05,$1F,$01,$28,$1A,$00,$38,$44,$06,$20,$02,$00,$1B,$00;82EBF8;      ;
                    ...
                    db $C8,$A4,$30,$2C,$1E,$2E,$00,$02,$C0,$B4,$2C,$2F,$23,$27,$00,$04;82F248;      ;
                                                            ;      ;      ;
SaveLoadMenu_StaticTileSourceOffsetTable:  dw $78C6,$78D0,$7908,$7910,$7A06,$7A10,$7A48,$7A50;82F258;      ;
                                                    ;      ;      ;
SaveLoadMenu_StaticTileVRAMDestTable:  dw $2002,$2006,$200A,$200C,$2022,$2026,$202A,$202C;82F268;      ;
                                                    ;      ;      ;
;;;;;;;; Pass 33: VRAM target positions for save-slot text fields.
;;;;;;;; Groups are used by name/year/day/date rendering for each menu slot.
SaveLoadMenu_SlotSummaryVRAMPositionTable:  dw $5010,$5030,$5050,$5060,$5110,$5130,$5150,$5160;82F278;      ;
                                                    ;      ;      ;
SaveLoadMenu_StaticTileBlockCountTable:  dw $0002,$0002,$0001,$0003,$0002,$0002,$0001,$0003;82F288;      ;
                                                            ;      ;      ;
SaveLoadMenu_YearOrdinalGlyphTable:
                dw $0012,$0013,$000D,$0003,$0011,$0003;82F298;      ;

; PASS44_POINTER42_PALETTE_SCRIPTS:
; Pointer42 color scripts are consumed by PaletteAnim_UpdatePointer42Slots. Normal commands are
; `dw color` + `db delay`; `$FFFE` jumps to a 24-bit script address; `$FFFF`
; clears the slot. The menu/status stages install these in Pointer42 slots 4-8.
FieldStatusMenu_PaletteAnim_Slot4Intro:
                dw $7F10
                db $20
                dw $7EF0
                db $10
                dw $7E8E
                db $10
FieldStatusMenu_PaletteAnim_Slot4Loop:
                dw $720A
                db $10
                dw $FFFE
                dl FieldStatusMenu_PaletteAnim_Slot4Loop

FieldStatusMenu_PaletteAnim_Slot5Intro:
                dw $7A8A
                db $10
                dw $724A
                db $10
                ...
                dw $65A3
                db $10
FieldStatusMenu_PaletteAnim_Slot5Loop:
                dw $58C3
                db $10
                dw $FFFE
                dl FieldStatusMenu_PaletteAnim_Slot5Loop

FieldStatusMenu_PaletteAnim_SelectedPulse:
                dw $629F
                db $08
                dw $5E1F
                db $08
                ...
                db $08
                dw $FFFE
                dl FieldStatusMenu_PaletteAnim_SelectedPulse

FieldStatusMenu_PaletteAnim_StaticHighlight:
                dw $7F3F
                db $08
                dw $FFFE
                dl FieldStatusMenu_PaletteAnim_StaticHighlight

IntroMenu_PaletteAnim_YellowPulse:
                dw $1D4D
                db $04
                dw $035D
                db $04
                ...
                db $04
                dw $FFFE
                dl IntroMenu_PaletteAnim_YellowPulse

IntroMenu_PaletteAnim_YellowStatic:
                dw $1D4D
                db $04
                dw $FFFE
                dl IntroMenu_PaletteAnim_YellowStatic

                db $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00;82F301;      ;
