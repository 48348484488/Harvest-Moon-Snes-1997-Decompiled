...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_AllocateAndInitNewSlot
;
; Core object/sprite allocator. Creates a new 36-byte GameOBJ entry in WRAM,
; selects the correct animation metadata bank ($86 or $87), loads the initial
; component list and reserves VRAM component slots.
;
; Inputs observed:
;   !gobj_sprite_table_idx = GameOBJ animation/metadata id
;   !gobj_pos_x / !gobj_pos_y = world/screen position used by the object
;   !gobj_flip_x = flip/attribute flags
; Output observed:
;   $A7 = 0 on success, $FFFF when no free slot is available
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GameOBJ_AllocateAndInitNewSlot:
    ...
    .slotfound:
        ...
        ASL A
        TAX
        LDA.L GameOBJ_AnimPtrTable_Bank86,X
        STA.B $75
        BRA .setupfinished
    ...
    .inbank87:
        ...
        ASL A
        TAX
        LDA.L GameOBJ_AnimPtrTable_Bank87,X
        STA.B $75
    ...
    .setupfinished:
        ...
        %Set16bit(!M)
        JSR.W GameOBJ_LoadComponentListAndAcquireVRAM
        %Set16bit(!M)
        LDA.W #$0000
        STA.B $A7
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_UpdateExistingSlotTransform
;
; Updates the transform fields of an already allocated GameOBJ slot selected by
; !gobj_last_gobj_used. This path does not rebuild the component/VRAM list.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GameOBJ_UpdateExistingSlotTransform:
        %Set16bit(!M)
        LDA.B $A1
        CMP.W #$0262
        BCS .GameOBJ_Bank85_Branch_8580CC
        %Set8bit(!M)
        LDA.B #$86
        STA.B $77
        STA.B $7A
        BRA .GameOBJ_Bank85_Branch_8580D4

    .GameOBJ_Bank85_Branch_8580CC:
        %Set8bit(!M)
        LDA.B #$87
        STA.B $77
        STA.B $7A

    .GameOBJ_Bank85_Branch_8580D4:
        %Set16bit(!MX)
        LDA.B $A5
        ASL A
        TAX
        LDA.L GameOBJ_SlotOffsetTable,X
        STA.B $A9
        TAX
        LDA.W #$0001
        STA.B $A7
        LDA.W $019C,X
        BEQ .GameOBJ_Bank85_Branch_8580FF
        LDA.B $9F
        STA.W $01A0,X
        LDA.B $9B
        STA.W $01A4,X
    ...
    .GameOBJ_Bank85_Branch_8580FF: RTL

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_ReinitializeExistingSlotMetadata
;
; Rewrites an existing slot with a new metadata id and rebuilds its component
; list. Used when an object keeps its slot but changes sprite/animation set.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GameOBJ_ReinitializeExistingSlotMetadata:
        %Set16bit(!MX)
        %Set16bit(!M)
        LDA.B $A1
        CMP.W #$0262
        BCS .GameOBJ_Bank85_Branch_858115
        %Set8bit(!M)
        LDA.B #$86
        STA.B $77
        STA.B $7A
        BRA .GameOBJ_Bank85_Branch_85811D

        .GameOBJ_Bank85_Branch_858115:
        %Set8bit(!M)
        LDA.B #$87
        STA.B $77
        STA.B $7A

        .GameOBJ_Bank85_Branch_85811D:
        %Set16bit(!MX)
        LDA.B $A5
        ASL A
        TAX
        LDA.L GameOBJ_SlotOffsetTable,X
        STA.B $A9
        JSR.W GameOBJ_CheckComponentVRAMSlotsReady
        LDA.B $AF
        BNE return85
        JSR.W GameOBJ_ReleaseComponentVRAMSlots
        %Set16bit(!MX)
        LDY.B $A5
        LDX.B $A9
        LDA.B $A1
        ...
        LDA.B $A1
        CMP.W #$0262
        BCS .GameOBJ_Bank85_Branch_858168
        LDA.W $019E,X
        ASL A
        TAX
        LDA.L GameOBJ_AnimPtrTable_Bank86,X
        STA.B $75
        BRA .GameOBJ_Bank85_Branch_858177

        .GameOBJ_Bank85_Branch_858168:
        LDA.W $019E,X
        SEC
        SBC.W #$0262
        ASL A
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_ClearSlotAndReleaseComponents
;
; Releases VRAM component references for the selected slot and marks the GameOBJ
; entry as unused by clearing its initialized marker.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        GameOBJ_ClearSlotAndReleaseComponents:
        %Set16bit(!MX)
        LDA.B $A5
        ...
        JSR.W GameOBJ_CheckComponentVRAMSlotsReady
        LDA.B $AF
        BEQ .GameOBJ_Bank85_Branch_8581B8
        JMP.W return85

        .GameOBJ_Bank85_Branch_8581B8:
        JSR.W GameOBJ_ReleaseComponentVRAMSlots
        %Set16bit(!MX)
        LDX.B $A9
        %Set16bit(!MX)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_LoadSlotStateToScratch
;
; Loads a GameOBJ slot back into scratch registers used by caller routines.
; This is commonly used before object-specific code updates position/state.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GameOBJ_LoadSlotStateToScratch:
        %Set16bit(!MX)
        LDA.B $A5
        ...
        TAX
        LDA.W $019C,X
        BEQ .GameOBJ_Bank85_Branch_8581F6
        LDA.W $019E,X
        STA.B $A1
        LDA.W $01A0,X
        STA.B $9F
    ...
    .GameOBJ_Bank85_Branch_8581F6:
        %Set8bit(!M)
        LDA.W $01AE,X
        %Set8bit(!M)
        CMP.B #$00
        BMI .GameOBJ_Bank85_Branch_858206
        XBA
        LDA.B #$00
        BRA .GameOBJ_Bank85_Branch_858209

    .GameOBJ_Bank85_Branch_858206:
        XBA
        LDA.B #$FF

    .GameOBJ_Bank85_Branch_858209:
        XBA
        %Set16bit(!M)
        STA.B $A7
...
;;;;;;;;
GameOBJ_UpdateAnimationFrames:
        %Set16bit(!MX)
        STZ.B $A9
        STZ.B $AB

    .GameOBJ_Bank85_Branch_8582CD:
            LDX.B $A9
            LDA.W $019C,X
            BNE .GameOBJ_Bank85_Branch_8582E7

        .GameOBJ_Bank85_Branch_8582D4:
            LDA.B $A9
            CLC
            ADC.W #$0024
            STA.B $A9
            ...
            LDA.B $AB
            CMP.B $DC
            BNE .GameOBJ_Bank85_Branch_8582CD

        JMP.W .GameOBJ_Bank85_Branch_858376

    .GameOBJ_Bank85_Branch_8582E7:
        %Set16bit(!MX)
        LDA.W $019E,X
        CMP.W #$0262
        BCS .GameOBJ_Bank85_Branch_8582FB
        %Set8bit(!M)
        LDA.B #$86
        STA.B $77
        STA.B $7A
        BRA .GameOBJ_Bank85_Branch_858303

    .GameOBJ_Bank85_Branch_8582FB:
        %Set8bit(!M)
        LDA.B #$87
        STA.B $77
        STA.B $7A

    .GameOBJ_Bank85_Branch_858303:
        %Set8bit(!M)
        LDA.W $01AE,X
        BNE .GameOBJ_Bank85_Branch_858349
        %Set16bit(!M)
        JSR.W GameOBJ_CheckComponentVRAMSlotsReady
        LDA.B $AF
        BNE .GameOBJ_Bank85_Branch_8582D4
        JSR.W GameOBJ_ReleaseComponentVRAMSlots
        %Set16bit(!MX)
        LDX.B $A9
        LDA.W $01AA,X
    ...
    .GameOBJ_Bank85_Branch_85831F:
        LDA.B [$75]
        BNE .GameOBJ_Bank85_Branch_85832A
        LDA.W $01A8,X
        STA.B $75
        BRA .GameOBJ_Bank85_Branch_85831F

    .GameOBJ_Bank85_Branch_85832A:
        STA.W $01AC,X
        %Set8bit(!M)
        LDY.W #$0002
        LDA.B [$75],Y
        ...
        JSR.W GameOBJ_LoadComponentListAndAcquireVRAM
        %Set16bit(!MX)
        BRA .GameOBJ_Bank85_Branch_858362

    .GameOBJ_Bank85_Branch_858349:
        %Set8bit(!M)
        LDA.W $01AE,X
        CMP.B #$FE
        BNE .GameOBJ_Bank85_Branch_858362
        %Set16bit(!M)
        JSR.W GameOBJ_ReleaseComponentVRAMSlots
        %Set16bit(!MX)
        LDA.W #$0000
        STA.W $019C,X
        JMP.W .GameOBJ_Bank85_Branch_8582D4

    .GameOBJ_Bank85_Branch_858362:
        LDX.B $A9
        %Set8bit(!M)
        LDA.W $01AE,X
        CMP.B #$FF
        BEQ .GameOBJ_Bank85_Branch_858371
        DEC A
        STA.W $01AE,X

    .GameOBJ_Bank85_Branch_858371:
        %Set16bit(!MX)
        JMP.W .GameOBJ_Bank85_Branch_8582D4

    .GameOBJ_Bank85_Branch_858376:
        RTL
    ...
    .gobjloop:
        ...
        .setnextcomponent:
            ...
            .flipyoffset:
                ...
                LSR A
                TAX
                LDA.L SOBJ_HighOAMXBitMaskTable,X
            ...
            .storingOAMcopy:
                ...
                ASL A
                TAX
                LDA.L SpriteComponent_VRAMTileIndexTable,X
                %Set8bit(!M)
                LDX.B !gobj_OAM_table_idx
...
;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; GameOBJ_LoadComponentListAndAcquireVRAM
;
; Reads a sprite component list from metadata banks $86/$87. Each component id
; is stored into the GameOBJ component array and passed to the VRAM slot manager.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
GameOBJ_LoadComponentListAndAcquireVRAM: ;858AE5
        %Set16bit(!MX)
        LDX.B !gobj_struct_idx
        LDA.W !gobj_struct_sprite_table_address,X
        STA.B $78
    ...
    .loop:
        ...
        .continue:
            ...
            PHX
            PHY
            JSR.W SpriteComponent_AcquireVRAMSlot
            %Set16bit(!MX)
            PLY
            PLX
            LDA.B $B3
...
;;;;;;;;
    GameOBJ_ReleaseComponentVRAMSlots:
        %Set16bit(!MX)
        LDX.B $A9
        %Set8bit(!M)
        LDA.B #$00
    ...
    .loop:
        ...
        PHY
        PHX
        JSR.W SpriteComponent_ReleaseVRAMSlot
        %Set8bit(!M)
        %Set16bit(!X)
        PLX
        PLY
...
GameOBJ_CheckComponentVRAMSlotsReady:
        %Set16bit(!X)
        LDX.B $A9
        %Set8bit(!M)
        LDA.B #$00
...
SOBJ_HighOAMXBitMaskTable: ;858BD0
        db $01,$00,$04,$00,$10,$00,$40,$00,$00,$01,$00,$04,$00,$10,$00,$40

;;;;;;;; GameOBJ slot offsets. Each GameOBJ struct is $24 bytes.
GameOBJ_SlotOffsetTable:;858BE0
...
;;;;;;;; TODO
SpriteComponent_AcquireVRAMSlot:
        %Set16bit(!MX)
        %Set8bit(!M)
        LDX.B $B3
        LDA.B #$00
...
;;;;;;; TODO
SpriteComponent_ReleaseVRAMSlot: ;858C9F
        %Set16bit(!MX)
        LDA.B $B3
        ASL A
        ASL A
...
SpriteComponent_BuildVRAMUploadQueue:
        %Set8bit(!M)
        LDA.B #128
        STA.B $7A
        %Set16bit(!MX)
    ...
    .loop:
        ...
        .continue2:
            ...
            ADC.W #$0004
            CMP.W #$0100
            BNE .GameOBJ_Bank85_Branch_858CF3
            LDA.W #$0000

        .GameOBJ_Bank85_Branch_858CF3:
            STA.B $BB
            LDA.W $0848
            AND.W #$003F
            INC A
            ...
            LDA.W $0846
            CMP.W #$0040
            BEQ .GameOBJ_Bank85_Branch_858D12
            STA.B $C5
            ASL A
            ASL A
            STA.B $C3
            INC.W $0846
            BRA .GameOBJ_Bank85_Branch_858D38

        .GameOBJ_Bank85_Branch_858D12:
            LDY.W #$0000
            LDA.W $0848
            ASL A
            ASL A
    ...
    .loop320:                                ;searches for a value
        ...
        .GameOBJ_Bank85_Branch_858D2F:
            BRA .GameOBJ_Bank85_Branch_858D2F                 ;infinite loop again???
        ...
        .GameOBJ_Bank85_Branch_858D38:
            LDA.B $C3
            BMI .GameOBJ_Bank85_Branch_858D38                 ;Another psible infinit loop!!!
            ...
            LDA.L $7EA322,X
            CMP.W #$FFFF
            BEQ .GameOBJ_Bank85_Branch_858D65
            PHX
            TAX
            %Set8bit(!M)
            LDA.B #$FF
    ...
    .escapeloop:
        ...
        LDA.B $BB
        CMP.B $BD
        BCS .GameOBJ_Bank85_Branch_858E37
        LDA.B $BD
        SEC
        SBC.B $BB
        BRA .GameOBJ_Bank85_Branch_858E3C

    .GameOBJ_Bank85_Branch_858E37:
        LDA.B $BB
        SEC
        SBC.B $BD

    .GameOBJ_Bank85_Branch_858E3C:
        LSR A
        LSR A
        STA.W $0742
        STZ.W $0743
